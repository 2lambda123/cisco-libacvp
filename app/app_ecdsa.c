...
void app_ecdsa_cleanup(void) {
  if (ecdsa_group_Qx)
    BN_free(ecdsa_group_Qx);
  ecdsa_group_Qx = NULL;
  if (ecdsa_group_Qy)
    BN_free(ecdsa_group_Qy);
  ecdsa_group_Qy = NULL;
  if (group_pkey)
    EVP_PKEY_free(group_pkey);
  group_pkey = NULL;
}
...
int app_ecdsa_handler(ACVP_TEST_CASE *test_case) {
  ...
  case ACVP_SUB_ECDSA_KEYVER:
    ...
    pub_key =
        ec_point_to_pub_key(tc->qx, tc->qx_len, tc->qy, tc->qy_len, &key_size);
    if (!pub_key) {
    ...
    OSSL_PARAM_BLD_push_utf8_string(pkey_pbld, OSSL_PKEY_PARAM_GROUP_NAME,
                                    curve, 0);
    OSSL_PARAM_BLD_push_octet_string(pkey_pbld, OSSL_PKEY_PARAM_PUB_KEY,
                                     pub_key, key_size);
    params = OSSL_PARAM_BLD_to_param(pkey_pbld);
    if (!params) {
      printf(
          "Error generating parameters for pkey generation in ECDSA keygen\n");
    }
  ...
  case ACVP_SUB_DET_ECDSA_SIGGEN:
    ...
    if (!tc->is_component) {
      ...
      if (EVP_DigestSignInit_ex(sig_ctx, NULL, md, NULL, NULL, group_pkey,
                                NULL) != 1) {
      ...
      if (alg == ACVP_SUB_DET_ECDSA_SIGGEN) {
        ...
        params = OSSL_PARAM_BLD_to_param(pkey_pbld);
        if (!params) {
          printf("Error generating parameters for pkey generation in DetECDSA "
                 "siggen\n");
        }
        ...
        pkey_ctx = NULL;
        if (EVP_DigestSignInit_ex(sig_ctx, &pkey_ctx, md, NULL, NULL,
                                  group_pkey, NULL) != 1) {
      ...
      sig_iter = sig; /* since d2i_ECDSA_SIG alters the pointer, we need to keep
                         the original one for freeing */
      if (EVP_DigestSign(sig_ctx, sig, &sig_len, tc->message, tc->msg_len) !=
          1) {
      ...
      sig_iter = sig; /* since d2i_ECDSA_SIG alters the pointer, we need to keep
                         the original one for freeing */
      if (EVP_PKEY_sign(comp_ctx, sig, &sig_len, tc->message, tc->msg_len) !=
          1) {
    ...
    sig_obj =
        d2i_ECDSA_SIG(NULL, (const unsigned char **)&sig_iter, (long)sig_len);
    if (!sig_obj) {
      printf("Error creating signature object neeed to retrieve output in "
             "ECDSA siggen\n");
      goto err;
    }
  ...
  case ACVP_SUB_ECDSA_SIGVER:
    ...
    if (!tc->is_component) {
      ...
      if (EVP_DigestVerifyInit_ex(sig_ctx, NULL, md, NULL, NULL, pkey, NULL) !=
          1) {
        printf("Error initializing signing for ECDSA sigver\n");
        goto err;
      }
      if (EVP_DigestVerify(sig_ctx, sig, sig_len, tc->message, tc->msg_len) ==
          1) {
        tc->ver_disposition = 1;
      }
    } else {
      ...
      if (EVP_PKEY_verify(comp_ctx, sig, sig_len, tc->message, tc->msg_len) ==
          1) {
        tc->ver_disposition = 1;
      }
    }
