...
static unsigned int base64_decode(const char *in, unsigned int inlen,
                                  unsigned char *out) {
  unsigned int i, j;
...
static int hmac_totp(const char *key, const unsigned char *msg, char *hash,
                     int hash_max, const EVP_MD *md, unsigned int key_len) {
  int len = 0;
...
#else
static int hmac_totp(const char *key, const unsigned char *msg, char *hash,
                     int hash_max, const char *md_name, unsigned int key_len) {
  int len = 0;
  unsigned char buff[MAX_LEN];

  EVP_Q_mac(NULL, "HMAC", NULL, md_name, NULL, key, key_len, msg, T_LEN, buff,
            MAX_LEN, (long unsigned int *)&len);
  memcpy_s(hash, hash_max, buff, len);
  return len;
}
#endif
...
#define MAX_SEED_LEN 64
  seed_len = base64_decode(seed, strnlen_s(seed, MAX_SEED_LEN),
...
#if OPENSSL_VERSION_NUMBER < 0x30000000L
  md_len = hmac_totp(new_seed, token_buff, hash, sizeof(hash), EVP_sha256(),
                     seed_len);
#else
  md_len =
      hmac_totp(new_seed, token_buff, hash, sizeof(hash), "SHA2-256", seed_len);
#endif
  if (md_len == 0) {
  ...
  bin = ((hash[os + 0] & 0x7f) << 24) | ((hash[os + 1] & 0xff) << 16) |
        ((hash[os + 2] & 0xff) << 8) | ((hash[os + 3] & 0xff) << 0);
...
int save_string_to_file(const char *str, const char *path) {
  ...
  if (fp && fclose(fp) == EOF) {
    printf("Encountered an error attempting to close output file. Cannot "
...
int get_nid_for_curve(ACVP_EC_CURVE curve) {
  ...
  case ACVP_EC_CURVE_P256:
    return NID_X9_62_prime256v1; /* OpenSSL omits the secp names since these are
                                    the same thing */
...
/** Convert the X and Y coordinates into the expected key format, since OpenSSL
 * does not allow setting X and Y directly */
char *ec_point_to_pub_key(unsigned char *x, int x_len, unsigned char *y,
                          int y_len, int *key_len) {
  int key_size = 0;
  char *key = NULL;
...
static const unsigned char sanity_msg[] = {0xA5, 0x30, 0xD4, 0x60, 0x93,
                                           0xA3, 0x5E, 0x50, 0x2C, 0xA1,
                                           0x64, 0xB7, 0x50, 0x24, 0xE4};

static const unsigned char sanity_hash[] = {
    0x13, 0x80, 0x22, 0xF7, 0xF3, 0xC6, 0xB9, 0x59, 0x36, 0x2D,
    0xFE, 0xAE, 0x59, 0xE9, 0xA3, 0x72, 0x24, 0x04, 0x3C, 0x61,
    0x1E, 0xE4, 0xAA, 0x01, 0xF0, 0xAA, 0x04, 0x2A};

/*
 * This performs a quick digest to make sure the FIPS provider is running
 * properly. Othewise, we will get a vague error trying to perform some
 * unrelated operation later on. The return code is the biggest indicator, but
 * we might as well check for correctness too.
 */
ACVP_RESULT fips_sanity_check() {
  ...
  if (EVP_Q_digest(NULL, "SHA2-224", "fips=yes", &sanity_msg, 15, md,
                   &md_len) != 1) {
...
static const unsigned char tdes_oid[] = {0x06, 0x0B, 0x2A, 0x86, 0x48,
                                         0x86, 0xF7, 0x0D, 0x01, 0x09,
                                         0x10, 0x03, 0x06};
static const unsigned char aes128_oid[] = {0x06, 0x09, 0x60, 0x86, 0x48, 0x01,
                                           0x65, 0x03, 0x04, 0x01, 0x05};
static const unsigned char aes192_oid[] = {0x06, 0x09, 0x60, 0x86, 0x48, 0x01,
                                           0x65, 0x03, 0x04, 0x01, 0x19};
static const unsigned char aes256_oid[] = {0x06, 0x09, 0x60, 0x86, 0x48, 0x01,
                                           0x65, 0x03, 0x04, 0x01, 0x2D};
