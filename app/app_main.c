...
static void setup_session_parameters(void) {
  ...
  if (!path_segment)
    path_segment = DEFAULT_URI_PREFIX;
...
static int verify_algorithms(APP_CONFIG *cfg) {
  ...
  /* If we are testing "all" then we don't need to tell the user they can't test
   * algs */
#if OPENSSL_VERSION_NUMBER < 0x30000000L
  if (!cfg->testall) {
    CHECK_NON_ALLOWED_ALG(
        cfg->dsa, "This version of OpenSSL does not support DSA testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->rsa, "This version of OpenSSL does not support RSA testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->drbg, "This version of OpenSSL does not support DRBG testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->ecdsa, "This version of OpenSSL does not support ECDSA testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->kas_ecc,
        "This version of OpenSSL does not support KAS-ECC testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->kas_ffc,
        "This version of OpenSSL does not support KAS-FFC testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->kas_ifc,
        "This version of OpenSSL does not support KAS-IFC testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->kda, "This version of OpenSSL does not support KDA testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->kts_ifc,
        "This version of OpenSSL does not support KTS-IFC testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->kdf, "This version of OpenSSL does not support KDF testing");
    CHECK_NON_ALLOWED_ALG(
        cfg->safe_primes,
        "This version of OpenSSL does not support safe primes testing");
  }
#endif
  ...
  if (!cfg->testall) {
    CHECK_NON_ALLOWED_ALG(
        cfg->dsa,
        "This version of OpenSSL does not support DSA testing (DSA disabled)");
    CHECK_NON_ALLOWED_ALG(cfg->kas_ffc,
                          "This version of OpenSSL does not support KAS-FFC "
                          "testing (DSA disabled)");
    CHECK_NON_ALLOWED_ALG(
        cfg->safe_primes,
        "This version of OpenSSL does not support safe primes testing");
  }
...
int main(int argc, char **argv) {
  ...
  if (!verify_algorithms(&cfg)) {
    printf("\nAlgorithm tests not supported by this crypto module have been "
           "requested. Please \n");
  ...
  if (!cfg.disable_fips) {
    ...
    /* Run a quick sanity check to determine that the FIPS provider is
     * functioning properly */
    rv = fips_sanity_check();
    if (rv != ACVP_SUCCESS) {
      printf("Error occured when testing FIPS at startup (rv = %d). Please "
             "verify the FIPS provider is\n",
             rv);
    ...
    printf("*******************************************************************"
           "****************\n");
    printf("* WARNING: You have chosen to not fetch the FIPS provider for this "
           "run. Any tests *\n");
    printf("* created or performed during this run MUST NOT have any "
           "validation requested     *\n");
    printf("* on it unless the FIPS provider is exclusively loaded or enabled "
           "by default in   *\n");
    printf("* your configuration. Proceed at your own risk. Continuing in 5 "
           "seconds...        *\n");
    printf("*******************************************************************"
           "****************\n");
    printf("\n");
  ...
  if (cfg.get) {
    rv = acvp_mark_as_get_only(ctx, cfg.get_string,
                               cfg.save_to ? cfg.save_file : NULL);
    if (rv != ACVP_SUCCESS) {
  ...
  if (!cfg.vector_req && cfg.vector_rsp) {
    printf("Offline vector processing requires both options, --vector_req and "
           "--vector_rsp\n");
    goto end;
  }
  ...
  if (cfg.manual_reg) {
    ...
    if (cfg.kas_ecc) {
      if (enable_kas_ecc(ctx))
        goto end;
    }
    if (cfg.kas_ifc) {
      if (enable_kas_ifc(ctx))
        goto end;
    }
    if (cfg.kts_ifc) {
      if (enable_kts_ifc(ctx))
        goto end;
    }
    if (cfg.kda) {
      if (enable_kda(ctx))
    ...
    if (cfg.kas_ffc) {
      if (enable_kas_ffc(ctx))
        goto end;
    }
    if (cfg.safe_primes) {
      if (enable_safe_primes(ctx))
        goto end;
    }
#endif
#endif
  ...
  if (cfg.get_cost) {
    ...
    if (diff < 0) {
      printf("Unable to get expected vector set count with given test session "
             "context.\n\n");
    } else {
      printf("The given test session context is expected to generate %d vector "
             "sets.\n\n",
             diff);
    }
    goto end;
  ...
  if (cfg.get_reg) {
    ...
    if (cfg.save_to) {
      if (save_string_to_file((const char *)reg,
                              (const char *)&cfg.save_file)) {
  ...
  if (cfg.vector_req && cfg.vector_rsp) {
    rv = acvp_run_vectors_from_file(ctx, cfg.vector_req_file,
                                    cfg.vector_rsp_file);
    goto end;
  }

  strncmp_s(DEFAULT_SERVER, DEFAULT_SERVER_LEN, server, DEFAULT_SERVER_LEN,
            &diff);
  if (!diff) {
    printf("Warning: No server set, using default. Please define ACV_SERVER in "
           "your environment.\n");
    printf("Run acvp_app --help for more information on this and other "
  ...
  /* PUT without algorithms submits put_filename for validation using save JWT
   * and testSession ID */
...
static int enable_aes(ACVP_CTX *ctx) {
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_KEYLEN,
                                    192);
...
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_EITHER);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_GCM,
                                      ACVP_SYM_CIPH_DOMAIN_IVLEN, 96, 1024, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_TAGLEN, 32);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_TAGLEN, 64);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_TAGLEN, 96);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_TAGLEN,
                                    104);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_TAGLEN,
                                    112);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_GCM,
                                      ACVP_SYM_CIPH_DOMAIN_PTLEN, 8, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_GCM,
                                      ACVP_SYM_CIPH_DOMAIN_AADLEN, 0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_DOMAIN_PTLEN, 0, 65536, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_DOMAIN_AADLEN, 0, 65536, 256);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GCM, ACVP_SYM_CIPH_TAGLEN,
                                    128);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_ECB, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_ECB,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_ECB, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_ECB, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC,
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(
      ctx, ACVP_AES_CBC_CS1, ACVP_SYM_CIPH_PARM_DIR, ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS1, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS1, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS1, ACVP_SYM_CIPH_KEYLEN,
                                    256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_CBC_CS1,
                                      ACVP_SYM_CIPH_DOMAIN_PTLEN, 128, 512, 8);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_sym_cipher_enable(ctx, ACVP_AES_CBC_CS2, &app_aes_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(
      ctx, ACVP_AES_CBC_CS2, ACVP_SYM_CIPH_PARM_DIR, ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS2, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS2, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS2, ACVP_SYM_CIPH_KEYLEN,
                                    256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_CBC_CS2,
                                      ACVP_SYM_CIPH_DOMAIN_PTLEN, 128, 512, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(
      ctx, ACVP_AES_CBC_CS3, ACVP_SYM_CIPH_PARM_DIR, ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS3, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS3, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CBC_CS3, ACVP_SYM_CIPH_KEYLEN,
                                    256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_CBC_CS3,
                                      ACVP_SYM_CIPH_DOMAIN_PTLEN, 136, 512, 8);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB1, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB1,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB1,
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB1, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB1, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB8, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB8,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB8,
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB8, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB8, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(
      ctx, ACVP_AES_CFB128, ACVP_SYM_CIPH_PARM_DIR, ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB128,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB128,
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB128, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CFB128, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_OFB, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_OFB,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_OFB,
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_OFB, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_OFB, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM,
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_KEYLEN,
                                    256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_CCM,
                                      ACVP_SYM_CIPH_DOMAIN_PTLEN, 0, 256, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_TAGLEN, 32);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_TAGLEN, 48);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_TAGLEN, 64);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_TAGLEN, 80);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_TAGLEN, 96);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_TAGLEN,
                                    112);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_TAGLEN,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_CCM,
                                      ACVP_SYM_CIPH_DOMAIN_IVLEN, 56, 104, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_CCM, ACVP_SYM_CIPH_DOMAIN_AADLEN, 0, 524288, 8);
  CHECK_ENABLE_CAP_RV(rv);

  /* AES-KW */
  rv = acvp_cap_sym_cipher_enable(ctx, ACVP_AES_KW, &app_aes_keywrap_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW,
                                    ...
                                    ACVP_SYM_CIPH_IVGEN_MODE_NA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW, ACVP_SYM_CIPH_KW_MODE,
                                    ACVP_SYM_KW_CIPHER);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW, ACVP_SYM_CIPH_KW_MODE,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_KW, ACVP_SYM_CIPH_DOMAIN_PTLEN, 128, 524288, 128);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_KW, ACVP_SYM_CIPH_DOMAIN_PTLEN, 128, 65536, 64);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW, ACVP_SYM_CIPH_KEYLEN, 128);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW, ACVP_SYM_CIPH_KEYLEN, 192);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KW, ACVP_SYM_CIPH_KEYLEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KWP, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KWP, ACVP_SYM_CIPH_KW_MODE,
                                    ACVP_SYM_KW_CIPHER);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KWP, ACVP_SYM_CIPH_KW_MODE,
                                    ACVP_SYM_KW_INVERSE);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KWP, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KWP, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_KWP, ACVP_SYM_CIPH_KEYLEN,
                                    256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_KWP,
                                      ACVP_SYM_CIPH_DOMAIN_PTLEN, 8, 524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_XTS, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_XTS, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_XTS, ACVP_SYM_CIPH_KEYLEN,
                                    256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_XTS, ACVP_SYM_CIPH_TWEAK,
                                    ACVP_SYM_CIPH_TWEAK_HEX);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_XTS, ACVP_SYM_CIPH_DOMAIN_PTLEN, 128, 65536, 128);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_XTS, ACVP_SYM_CIPH_DOMAIN_PTLEN, 256, 65536, 256);
  CHECK_ENABLE_CAP_RV(rv);
#endif
#if 0
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);

  // CTR_INCR and CTR_OVRFLW are ignored by server if PERFORM_CTR is false - can
  // remove those calls if so
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR,
                                    ACVP_SYM_CIPH_PARM_PERFORM_CTR, 1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR,
                                    ACVP_SYM_CIPH_PARM_CTR_INCR, 1);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR,
                                    ACVP_SYM_CIPH_PARM_CTR_OVRFLW, 0);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR,
                                    ACVP_SYM_CIPH_PARM_CTR_OVRFLW, 1);
  CHECK_ENABLE_CAP_RV(rv);
#endif
...
#endif
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_CTR, ACVP_SYM_CIPH_KEYLEN,
                                    256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_CTR,
                                      ACVP_SYM_CIPH_DOMAIN_PTLEN, 8, 128, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC,
                                    ACVP_SYM_CIPH_PARM_IVGEN_SRC,
                                    ACVP_SYM_CIPH_IVGEN_SRC_EITHER);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_TAGLEN,
                                    32);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_TAGLEN,
                                    64);
  ...
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_TAGLEN,
                                    120);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(ctx, ACVP_AES_GMAC,
                                      ACVP_SYM_CIPH_DOMAIN_AADLEN, 0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_domain(
      ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_DOMAIN_AADLEN, 256, 65536, 256);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC,
                                    ACVP_SYM_CIPH_PARM_IVGEN_MODE,
                                    ACVP_SYM_CIPH_IVGEN_MODE_821);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_KEYLEN,
                                    128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_KEYLEN,
                                    192);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_sym_cipher_set_parm(ctx, ACVP_AES_GMAC, ACVP_SYM_CIPH_IVLEN, 96);
  CHECK_ENABLE_CAP_RV(rv);
...
static int enable_tdes(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_ECB, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_ECB, ACVP_SYM_CIPH_PARM_KO,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_CBC, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_CBC, ACVP_SYM_CIPH_PARM_KO,
                                    ACVP_SYM_CIPH_KO_ONE);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_OFB, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_OFB, ACVP_SYM_CIPH_PARM_KO,
                                    ACVP_SYM_CIPH_KO_ONE);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(
      ctx, ACVP_TDES_CFB64, ACVP_SYM_CIPH_PARM_DIR, ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_CFB64, ACVP_SYM_CIPH_PARM_KO,
                                    ACVP_SYM_CIPH_KO_ONE);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_CFB8, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_CFB8, ACVP_SYM_CIPH_PARM_KO,
                                    ACVP_SYM_CIPH_KO_ONE);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_CFB1, ACVP_SYM_CIPH_PARM_DIR,
                                    ACVP_SYM_CIPH_DIR_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_sym_cipher_set_parm(ctx, ACVP_TDES_CFB1, ACVP_SYM_CIPH_PARM_KO,
                                    ACVP_SYM_CIPH_KO_ONE);
  CHECK_ENABLE_CAP_RV(rv);
#endif
...
static int enable_hash(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA1, ACVP_HASH_MESSAGE_LEN, 0,
                                65536, 8);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_hash_enable(ctx, ACVP_HASH_SHA224, &app_sha_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA224, ACVP_HASH_MESSAGE_LEN, 0,
                                65536, 8);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_hash_enable(ctx, ACVP_HASH_SHA256, &app_sha_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA256, ACVP_HASH_MESSAGE_LEN, 0,
                                65536, 8);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_hash_enable(ctx, ACVP_HASH_SHA384, &app_sha_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA384, ACVP_HASH_MESSAGE_LEN, 0,
                                65536, 8);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_hash_enable(ctx, ACVP_HASH_SHA512, &app_sha_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA512, ACVP_HASH_MESSAGE_LEN, 0,
                                65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA512_224,
                                ACVP_HASH_MESSAGE_LEN, 0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_hash_enable(ctx, ACVP_HASH_SHA512_256, &app_sha_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA512_256,
                                ACVP_HASH_MESSAGE_LEN, 0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA3_224, ACVP_HASH_MESSAGE_LEN,
                                0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA3_256, ACVP_HASH_MESSAGE_LEN,
                                0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA3_384, ACVP_HASH_MESSAGE_LEN,
                                0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHA3_512, ACVP_HASH_MESSAGE_LEN,
                                0, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHAKE_128, ACVP_HASH_OUT_LENGTH,
                                16, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_domain(ctx, ACVP_HASH_SHAKE_256, ACVP_HASH_OUT_LENGTH,
                                16, 65536, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_hash_set_parm(ctx, ACVP_HASH_SHA512_224, ACVP_HASH_LARGE_DATA,
                              1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_parm(ctx, ACVP_HASH_SHA512_224, ACVP_HASH_LARGE_DATA,
                              2);
  ...
  rv = acvp_cap_hash_set_parm(ctx, ACVP_HASH_SHA512_224, ACVP_HASH_LARGE_DATA,
                              8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_parm(ctx, ACVP_HASH_SHA512_256, ACVP_HASH_LARGE_DATA,
                              1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hash_set_parm(ctx, ACVP_HASH_SHA512_256, ACVP_HASH_LARGE_DATA,
                              2);
...
static int enable_cmac(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_cmac_set_domain(ctx, ACVP_CMAC_AES, ACVP_CMAC_MSGLEN, 0, 524288,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_cmac_set_domain(ctx, ACVP_CMAC_TDES, ACVP_CMAC_MSGLEN, 0, 65536,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
...
static int enable_kmac(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_domain(ctx, ACVP_KMAC_128, ACVP_KMAC_MSGLEN, 0, 65536,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_domain(ctx, ACVP_KMAC_128, ACVP_KMAC_MACLEN, 32, 65536,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_domain(ctx, ACVP_KMAC_128, ACVP_KMAC_KEYLEN, 128, 1024,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_parm(ctx, ACVP_KMAC_128, ACVP_KMAC_XOF_SUPPORT,
                              ACVP_XOF_SUPPORT_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  /* OpenSSL 3.X supports hex customization strings, but they are not on the
   * FIPS cert, so leaving disabled */
  rv = acvp_cap_kmac_set_parm(ctx, ACVP_KMAC_128, ACVP_KMAC_HEX_CUSTOM_SUPPORT,
                              0);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_kmac_enable(ctx, ACVP_KMAC_256, &app_kmac_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_domain(ctx, ACVP_KMAC_256, ACVP_KMAC_MSGLEN, 0, 65536,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_domain(ctx, ACVP_KMAC_256, ACVP_KMAC_MACLEN, 32, 65536,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_domain(ctx, ACVP_KMAC_256, ACVP_KMAC_KEYLEN, 128, 1024,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kmac_set_parm(ctx, ACVP_KMAC_256, ACVP_KMAC_XOF_SUPPORT,
                              ACVP_XOF_SUPPORT_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
  /* OpenSSL 3.X supports hex customization strings, but they are not on the
   * FIPS cert, so leaving disabled */
  rv = acvp_cap_kmac_set_parm(ctx, ACVP_KMAC_256, ACVP_KMAC_HEX_CUSTOM_SUPPORT,
                              0);
  CHECK_ENABLE_CAP_RV(rv);
end:
  return rv;
...
static int enable_hmac(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA1, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA1, ACVP_HMAC_MACLEN, 32, 160,
                                8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_224, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_224, ACVP_HMAC_MACLEN, 32,
                                224, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_256, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_256, ACVP_HMAC_MACLEN, 32,
                                256, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_384, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_384, ACVP_HMAC_MACLEN, 32,
                                384, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_512, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_512, ACVP_HMAC_MACLEN, 32,
                                512, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_512_224, ACVP_HMAC_KEYLEN,
                                8, 524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_512_224, ACVP_HMAC_MACLEN,
                                32, 224, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_512_256, ACVP_HMAC_KEYLEN,
                                8, 524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA2_512_256, ACVP_HMAC_MACLEN,
                                32, 256, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_224, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_224, ACVP_HMAC_MACLEN, 32,
                                224, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_256, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_256, ACVP_HMAC_MACLEN, 32,
                                256, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_384, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_384, ACVP_HMAC_MACLEN, 32,
                                384, 8);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_512, ACVP_HMAC_KEYLEN, 8,
                                524288, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_hmac_set_domain(ctx, ACVP_HMAC_SHA3_512, ACVP_HMAC_MACLEN, 32,
                                512, 8);
  CHECK_ENABLE_CAP_RV(rv);
...
static int enable_kdf(ACVP_CTX *ctx) {
  ...
  flags = ACVP_SHA1 | ACVP_SHA224 | ACVP_SHA256 | ACVP_SHA384 | ACVP_SHA512;

  rv = acvp_cap_kdf135_ssh_set_parm(ctx, ACVP_KDF135_SSH,
                                    ACVP_SSH_METH_TDES_CBC, flags);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_ssh_set_parm(ctx, ACVP_KDF135_SSH,
                                    ACVP_SSH_METH_AES_128_CBC, flags);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_ssh_set_parm(ctx, ACVP_KDF135_SSH,
                                    ACVP_SSH_METH_AES_192_CBC, flags);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_ssh_set_parm(ctx, ACVP_KDF135_SSH,
                                    ACVP_SSH_METH_AES_256_CBC, flags);
  CHECK_ENABLE_CAP_RV(rv);
#if 0
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_KDF_TYPE,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_HASH_ALG,
                                     ACVP_SHA512_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_HASH_ALG,
                                     ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_HASH_ALG, ACVP_SHA3_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_HASH_ALG, ACVP_SHA3_256);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_HASH_ALG, ACVP_SHA3_384);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_HASH_ALG, ACVP_SHA3_512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_domain(ctx, ACVP_KDF_X942_KEY_LEN, 8, 4096, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_domain(ctx, ACVP_KDF_X942_OTHER_INFO_LEN, 0,
                                       4096, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_domain(ctx, ACVP_KDF_X942_ZZ_LEN, 8, 4096, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_domain(ctx, ACVP_KDF_X942_SUPP_INFO_LEN, 0, 120,
                                       8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf135_x942_set_parm(ctx, ACVP_KDF_X942_OID,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_COUNTER,
                                ACVP_KDF108_SUPPORTED_LEN, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_COUNTER,
                                ACVP_KDF108_SUPPORTED_LEN, 72);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_COUNTER,
                                ACVP_KDF108_SUPPORTED_LEN, 128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_COUNTER,
                                ACVP_KDF108_SUPPORTED_LEN, 776);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_COUNTER,
                                ACVP_KDF108_SUPPORTED_LEN, 3456);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_COUNTER,
                                ACVP_KDF108_SUPPORTED_LEN, 4096);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_COUNTER,
  ...
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_FEEDBACK,
                                ACVP_KDF108_SUPPORTED_LEN, 8);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_FEEDBACK,
                                ACVP_KDF108_SUPPORTED_LEN, 72);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_FEEDBACK,
                                ACVP_KDF108_SUPPORTS_EMPTY_IV, 1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_FEEDBACK,
                                ACVP_KDF108_REQUIRES_EMPTY_IV, 1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf108_set_parm(ctx, ACVP_KDF108_MODE_FEEDBACK,
                                ACVP_KDF108_COUNTER_LEN, 32);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kdf_tls13_set_parm(ctx, ACVP_KDF_TLS13_RUNNING_MODE,
...
static int enable_kas_ecc(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                   ACVP_PREREQ_ECDSA, value);
  CHECK_ENABLE_CAP_RV(rv);
#ifdef ACVP_ENABLE_DEPRECATED_VERSION
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_FUNCTION,
                                 ACVP_KAS_ECC_FUNC_PARTIAL);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
...
#endif
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_P224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_P256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_P384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_P521);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_K233);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_K283);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_K409);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_K571);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_B233);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_B283);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_B409);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_CDH, ACVP_KAS_ECC_MODE_CDH,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_B571);
  CHECK_ENABLE_CAP_RV(rv);

#ifdef ACVP_ENABLE_DEPRECATED_VERSION
  rv = acvp_cap_kas_ecc_enable(ctx, ACVP_KAS_ECC_COMP, &app_kas_ecc_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(ctx, ACVP_KAS_ECC_COMP,
                                   ACVP_KAS_ECC_MODE_COMPONENT,
                                   ACVP_PREREQ_ECDSA, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(ctx, ACVP_KAS_ECC_COMP,
                                   ACVP_KAS_ECC_MODE_COMPONENT, ACVP_PREREQ_SHA,
                                   value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(ctx, ACVP_KAS_ECC_COMP,
                                   ACVP_KAS_ECC_MODE_COMPONENT,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(ctx, ACVP_KAS_ECC_COMP,
                                   ACVP_KAS_ECC_MODE_COMPONENT, ACVP_PREREQ_CCM,
                                   value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(ctx, ACVP_KAS_ECC_COMP,
                                   ACVP_KAS_ECC_MODE_COMPONENT,
                                   ACVP_PREREQ_CMAC, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(ctx, ACVP_KAS_ECC_COMP,
                                   ACVP_KAS_ECC_MODE_COMPONENT,
                                   ACVP_PREREQ_HMAC, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(
      ctx, ACVP_KAS_ECC_COMP, ACVP_KAS_ECC_MODE_COMPONENT,
      ACVP_KAS_ECC_FUNCTION, ACVP_KAS_ECC_FUNC_PARTIAL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(
      ctx, ACVP_KAS_ECC_COMP, ACVP_KAS_ECC_MODE_COMPONENT,
      ACVP_KAS_ECC_EPHEMERAL_UNIFIED, ACVP_KAS_ECC_ROLE, 0,
      ACVP_KAS_ECC_ROLE_INITIATOR);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(
      ctx, ACVP_KAS_ECC_COMP, ACVP_KAS_ECC_MODE_COMPONENT,
      ACVP_KAS_ECC_EPHEMERAL_UNIFIED, ACVP_KAS_ECC_ROLE, 0,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(ctx, ACVP_KAS_ECC_COMP,
                                   ACVP_KAS_ECC_MODE_COMPONENT,
                                   ACVP_KAS_ECC_EPHEMERAL_UNIFIED,
                                   ACVP_KAS_ECC_KDF, 0, ACVP_KAS_ECC_NOKDFNOKC);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(
      ctx, ACVP_KAS_ECC_COMP, ACVP_KAS_ECC_MODE_COMPONENT,
      ACVP_KAS_ECC_EPHEMERAL_UNIFIED, ACVP_KAS_ECC_EB, ACVP_EC_CURVE_P224,
      ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(
      ctx, ACVP_KAS_ECC_COMP, ACVP_KAS_ECC_MODE_COMPONENT,
      ACVP_KAS_ECC_EPHEMERAL_UNIFIED, ACVP_KAS_ECC_EC, ACVP_EC_CURVE_P256,
      ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(
      ctx, ACVP_KAS_ECC_COMP, ACVP_KAS_ECC_MODE_COMPONENT,
      ACVP_KAS_ECC_EPHEMERAL_UNIFIED, ACVP_KAS_ECC_ED, ACVP_EC_CURVE_P384,
      ACVP_SHA384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(
      ctx, ACVP_KAS_ECC_COMP, ACVP_KAS_ECC_MODE_COMPONENT,
      ACVP_KAS_ECC_EPHEMERAL_UNIFIED, ACVP_KAS_ECC_EE, ACVP_EC_CURVE_P521,
      ACVP_SHA512);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(
      ctx, ACVP_KAS_ECC_SSC, ACVP_KAS_ECC_MODE_NONE, ACVP_PREREQ_ECDSA, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(
      ctx, ACVP_KAS_ECC_SSC, ACVP_KAS_ECC_MODE_NONE, ACVP_PREREQ_SHA, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_prereq(
      ctx, ACVP_KAS_ECC_SSC, ACVP_KAS_ECC_MODE_NONE, ACVP_PREREQ_DRBG, value);
  CHECK_ENABLE_CAP_RV(rv);
#if 0
...
#endif
  rv = acvp_cap_kas_ecc_set_scheme(
      ctx, ACVP_KAS_ECC_SSC, ACVP_KAS_ECC_MODE_NONE,
      ACVP_KAS_ECC_EPHEMERAL_UNIFIED, ACVP_KAS_ECC_ROLE, 0,
      ACVP_KAS_ECC_ROLE_INITIATOR);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_scheme(
      ...
      ACVP_KAS_ECC_ROLE_RESPONDER);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_SSC, ACVP_KAS_ECC_MODE_NONE,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_P224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ecc_set_parm(ctx, ACVP_KAS_ECC_SSC, ACVP_KAS_ECC_MODE_NONE,
                                 ACVP_KAS_ECC_CURVE, ACVP_EC_CURVE_P256);
...
static int enable_kas_ifc(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_KAS1,
                                 ACVP_KAS_IFC_INITIATOR);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_KAS1,
                                 ACVP_KAS_IFC_RESPONDER);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_MODULO,
                                 2048);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_MODULO,
                                 3072);
  ...
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_MODULO,
                                 4096);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_exponent(ctx, ACVP_KAS_IFC_SSC,
                                     ACVP_KAS_IFC_FIXEDPUBEXP, expo_str);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_KAS2,
                                 ACVP_KAS_IFC_INITIATOR);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_KAS2,
                                 ACVP_KAS_IFC_RESPONDER);
  ...
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC, ACVP_KAS_IFC_MODULO,
                                 8192);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC,
                                 ACVP_KAS_IFC_KEYGEN_METHOD,
                                 ACVP_KAS_IFC_RSAKPG1_BASIC);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ifc_set_parm(ctx, ACVP_KAS_IFC_SSC,
                                 ACVP_KAS_IFC_KEYGEN_METHOD,
...
static int enable_kts_ifc(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_param_string(ctx, ACVP_KTS_IFC,
                                         ACVP_KTS_IFC_FIXEDPUBEXP, expo_str);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_param_string(ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_IUT_ID,
                                         iut_id);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_parm(ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_FUNCTION,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_parm(ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_KEYGEN_METHOD,
                                 ACVP_KTS_IFC_RSAKPG1_BASIC);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_parm(ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_KEYGEN_METHOD,
  ...
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_kts_ifc_set_parm(ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_SCHEME,
                                 ACVP_KTS_IFC_KAS1_BASIC);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(
      ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_KAS1_BASIC, ACVP_KTS_IFC_ROLE,
      ACVP_KTS_IFC_RESPONDER);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(
      ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_KAS1_BASIC, ACVP_KTS_IFC_ROLE,
      ACVP_KTS_IFC_INITIATOR);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA512_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA3_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA3_256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA3_384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
                                        ACVP_KTS_IFC_HASH, ACVP_SHA3_512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_parm(
      ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_KAS1_BASIC, ACVP_KTS_IFC_L, 1024);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_kts_ifc_set_scheme_parm(
      ctx, ACVP_KTS_IFC, ACVP_KTS_IFC_KAS1_BASIC, ACVP_KTS_IFC_L, 512);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_kts_ifc_set_scheme_parm(ctx, ACVP_KTS_IFC,
                                        ACVP_KTS_IFC_KAS1_BASIC,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kts_ifc_set_scheme_string(ctx, ACVP_KTS_IFC,
                                          ACVP_KTS_IFC_KAS1_BASIC,
                                          ACVP_KTS_IFC_ENCODING, concatenation);
  CHECK_ENABLE_CAP_RV(rv);
...
static int enable_kas_ffc(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_COMP,
                                   ACVP_KAS_FFC_MODE_NONE,
                                   ACVP_PREREQ_SAFE_PRIMES, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_COMP,
                                   ACVP_KAS_FFC_MODE_COMPONENT, ACVP_PREREQ_DSA,
                                   value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_COMP,
                                   ACVP_KAS_FFC_MODE_COMPONENT, ACVP_PREREQ_SHA,
                                   value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_COMP,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_COMP,
                                   ACVP_KAS_FFC_MODE_COMPONENT, ACVP_PREREQ_CCM,
                                   value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_COMP,
                                   ACVP_KAS_FFC_MODE_COMPONENT,
  ...
  rv = acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_COMP,
                                   ...
                                   ACVP_PREREQ_HMAC, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(
      ctx, ACVP_KAS_FFC_COMP, ACVP_KAS_FFC_MODE_COMPONENT,
      ACVP_KAS_FFC_FUNCTION, ACVP_KAS_FFC_FUNC_DPGEN);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(
      ctx, ACVP_KAS_FFC_COMP, ACVP_KAS_FFC_MODE_COMPONENT,
      ACVP_KAS_FFC_FUNCTION, ACVP_KAS_FFC_FUNC_DPVAL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_scheme(ctx, ACVP_KAS_FFC_COMP,
                                   ACVP_KAS_FFC_MODE_COMPONENT,
                                   ACVP_KAS_FFC_DH_EPHEMERAL, ACVP_KAS_FFC_ROLE,
                                   ACVP_KAS_FFC_ROLE_INITIATOR);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_scheme(ctx, ACVP_KAS_FFC_COMP,
                                   ACVP_KAS_FFC_MODE_COMPONENT,
                                   ACVP_KAS_FFC_DH_EPHEMERAL, ACVP_KAS_FFC_ROLE,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_scheme(
      ctx, ACVP_KAS_FFC_COMP, ACVP_KAS_FFC_MODE_COMPONENT,
      ACVP_KAS_FFC_DH_EPHEMERAL, ACVP_KAS_FFC_KDF, ACVP_KAS_FFC_NOKDFNOKC);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_scheme(
      ctx, ACVP_KAS_FFC_COMP, ACVP_KAS_FFC_MODE_COMPONENT,
      ACVP_KAS_FFC_DH_EPHEMERAL, ACVP_KAS_FFC_FB, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_scheme(
      ctx, ACVP_KAS_FFC_COMP, ACVP_KAS_FFC_MODE_COMPONENT,
      ACVP_KAS_FFC_DH_EPHEMERAL, ACVP_KAS_FFC_FC, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_scheme(
      ctx, ACVP_KAS_FFC_COMP, ACVP_KAS_FFC_MODE_COMPONENT,
      ACVP_KAS_FFC_DH_EPHEMERAL, ACVP_KAS_FFC_FB, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_kas_ffc_set_prereq(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                  ACVP_PREREQ_SAFE_PRIMES, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(
      ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE, ACVP_PREREQ_DSA, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(
      ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE, ACVP_PREREQ_SHA, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_prereq(
      ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE, ACVP_PREREQ_DRBG, value);
  CHECK_ENABLE_CAP_RV(rv);
#if 0
...
#endif
  rv = acvp_cap_kas_ffc_set_scheme(
      ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE, ACVP_KAS_FFC_DH_EPHEMERAL,
      ACVP_KAS_FFC_ROLE, ACVP_KAS_FFC_ROLE_INITIATOR);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_scheme(
      ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE, ACVP_KAS_FFC_DH_EPHEMERAL,
      ACVP_KAS_FFC_ROLE, ACVP_KAS_FFC_ROLE_RESPONDER);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_FC);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_FB);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER < 0x30000000L
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_MODP2048);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_MODP3072);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_MODP4096);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_MODP6144);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_MODP8192);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_FFDHE2048);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_FFDHE3072);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_FFDHE4096);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_FFDHE6144);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kas_ffc_set_parm(ctx, ACVP_KAS_FFC_SSC, ACVP_KAS_FFC_MODE_NONE,
                                 ACVP_KAS_FFC_GEN_METH, ACVP_KAS_FFC_FFDHE8192);
  CHECK_ENABLE_CAP_RV(rv);
end:
...
static int enable_kda(ACVP_CTX *ctx) {
...
#endif
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_ALGID, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_L, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_UPARTYINFO, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_VPARTYINFO, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG, ACVP_SHA1,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_domain(ctx, ACVP_KDA_HKDF, ACVP_KDA_Z, 224, 8192, 8);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_REVISION,
                             ACVP_REVISION_SP800_56CR1, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_LITERAL, "0123456789ABCDEF");
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_UPARTYINFO, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_CONTEXT, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_ALGID, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_LABEL, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_L, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_ENCODING_TYPE,
                             ACVP_KDA_ENCODING_CONCAT, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG, ACVP_SHA224,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG, ACVP_SHA256,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG, ACVP_SHA384,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG, ACVP_SHA512,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG,
                             ACVP_SHA512_224, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG,
                             ACVP_SHA512_256, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG,
                             ACVP_SHA3_224, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_ALG,
                             ACVP_SHA3_512, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_SALT,
                             ACVP_KDA_MAC_SALT_METHOD_DEFAULT, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_HKDF, ACVP_KDA_MAC_SALT,
                             ACVP_KDA_MAC_SALT_METHOD_RANDOM, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_ALGID, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_L, NULL);
  ...
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_VPARTYINFO, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA512,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA2_224,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                            ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_KMAC_128, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_domain(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_Z, 224, 8192, 8);
  CHECK_ENABLE_CAP_RV(rv);
#else
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_REVISION,
                             ACVP_REVISION_SP800_56CR1, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_PATTERN,
                             ACVP_KDA_PATTERN_LITERAL, "0123456789ABCDEF");
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA224,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA256,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA384,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA512,
  ...
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ...
                             ACVP_HASH_SHA512_224, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION,
                             ACVP_HASH_SHA512_256, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA3_224,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA3_256,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA3_384,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HASH_SHA3_512,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA2_224,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA2_256,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA2_384,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA2_512,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION,
                             ACVP_HMAC_SHA2_512_224, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION,
                             ACVP_HMAC_SHA2_512_256, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA3_224,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA3_256,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA3_384,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP,
                             ACVP_KDA_ONESTEP_AUX_FUNCTION, ACVP_HMAC_SHA3_512,
                             NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_domain(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_Z, 224, 1024, 8);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_ENCODING_TYPE,
                             ACVP_KDA_ENCODING_CONCAT, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_MAC_SALT,
                             ACVP_KDA_MAC_SALT_METHOD_DEFAULT, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_set_parm(ctx, ACVP_KDA_ONESTEP, ACVP_KDA_MAC_SALT,
                             ACVP_KDA_MAC_SALT_METHOD_RANDOM, NULL);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_PATTERN,
                                     ACVP_KDA_PATTERN_ALGID, 0, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_PATTERN, ACVP_KDA_PATTERN_L,
                                     0, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_PATTERN,
                                     ACVP_KDA_PATTERN_UPARTYINFO, 0, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_PATTERN,
                                     ACVP_KDA_PATTERN_VPARTYINFO, 0, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_ENCODING_TYPE,
                                     ACVP_KDA_ENCODING_CONCAT,
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_MAC_SALT,
  ...
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_MAC_ALG,
                                     ACVP_KDF108_MAC_MODE_HMAC_SHA1,
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_MAC_ALG,
  ...
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_MAC_ALG,
                                     ...
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_TWOSTEP_FIXED_DATA_ORDER,
                                     ACVP_KDF108_FIXED_DATA_ORDER_AFTER,
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_TWOSTEP_COUNTER_LEN, 8,
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_TWOSTEP_SUPPORTS_EMPTY_IV, 1,
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_TWOSTEP_REQUIRES_EMPTY_IV, 1,
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_kda_twostep_set_parm(ctx, ACVP_KDA_TWOSTEP_SUPPORTED_LEN, 2048,
                                     ACVP_KDF108_MODE_FEEDBACK, NULL);
  CHECK_ENABLE_CAP_RV(rv);
#endif
...
static int enable_dsa(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_GENPQ, ACVP_DSA_PROBABLE);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_GENG, ACVP_DSA_CANONICAL);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_GENG, ACVP_DSA_UNVERIFIABLE);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA512);
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA512_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGGEN, ACVP_DSA_MODE_PQGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
#endif
#endif
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_GENPQ, ACVP_DSA_PROBABLE);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_GENG, ACVP_DSA_CANONICAL);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA512_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_PQGVER, ACVP_DSA_MODE_PQGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_KEYGEN, ACVP_DSA_MODE_KEYGEN,
                             ACVP_DSA_LN2048_224, ACVP_NO_SHA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_KEYGEN, ACVP_DSA_MODE_KEYGEN,
                             ACVP_DSA_LN2048_256, ACVP_NO_SHA);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_KEYGEN, ACVP_DSA_MODE_KEYGEN,
                             ACVP_DSA_LN3072_256, ACVP_NO_SHA);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN2048_224, ACVP_SHA256);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA256);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA512_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN2048_256, ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA512_224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGGEN, ACVP_DSA_MODE_SIGGEN,
                             ACVP_DSA_LN3072_256, ACVP_SHA512_256);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGVER, ACVP_DSA_MODE_SIGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_dsa_set_parm(ctx, ACVP_DSA_SIGVER, ACVP_DSA_MODE_SIGVER,
                             ACVP_DSA_LN1024_160, ACVP_SHA256);
...
static int enable_rsa(ACVP_CTX *ctx) {
  ...
  /* Enable RSA keygen... */
  rv =
      acvp_cap_rsa_keygen_enable(ctx, ACVP_RSA_KEYGEN, &app_rsa_keygen_handler);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_keygen_set_parm(ctx, ACVP_RSA_PARM_KEY_FORMAT,
                                    ACVP_RSA_KEY_FORMAT_CRT);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_keygen_set_parm(ctx, ACVP_RSA_PARM_PUB_EXP_MODE,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 2048,
                                        ACVP_SHA256, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 2048,
                                        ACVP_SHA384, 0);
  ...
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 2048,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 3072,
                                        ACVP_SHA256, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 3072,
                                        ACVP_SHA384, 0);
  ...
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 3072,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 4096,
                                        ACVP_SHA256, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 4096,
                                        ACVP_SHA384, 0);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 2048,
                                        ACVP_SHA224, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 2048,
                                        ACVP_SHA256, 0);
  ...
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 2048,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 3072,
                                        ACVP_SHA224, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 3072,
                                        ACVP_SHA256, 0);
  ...
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 3072,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 4096,
                                        ACVP_SHA224, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 4096,
                                        ACVP_SHA256, 0);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 2048,
                                        ACVP_SHA224, 24);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 2048,
                                        ACVP_SHA256, 32);
  ...
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 2048,
                                        ACVP_SHA512, 64);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 3072,
                                        ACVP_SHA224, 24);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 3072,
                                        ACVP_SHA256, 32);
  ...
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 3072,
                                        ACVP_SHA512, 64);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 4096,
                                        ACVP_SHA224, 24);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_siggen_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 4096,
                                        ACVP_SHA256, 32);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_parm(ctx, ACVP_RSA_PARM_PUB_EXP_MODE,
                                    ACVP_RSA_PUB_EXP_MODE_RANDOM);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 1024,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 1024,
                                        ACVP_SHA256, 0);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 1024,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 2048,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 2048,
                                        ACVP_SHA256, 0);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 2048,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 3072,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 3072,
                                        ACVP_SHA256, 0);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 3072,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 4096,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_X931, 4096,
                                        ACVP_SHA256, 0);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 1024,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 1024,
                                        ACVP_SHA224, 0);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 1024,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 2048,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 2048,
                                        ACVP_SHA224, 0);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 2048,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 3072,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 3072,
                                        ACVP_SHA224, 0);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 3072,
                                        ACVP_SHA512, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 4096,
                                        ACVP_SHA1, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1V15, 4096,
                                        ACVP_SHA224, 0);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 1024,
                                        ACVP_SHA1, 20);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 1024,
                                        ACVP_SHA224, 24);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 1024,
                                        ACVP_SHA512, 62);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 2048,
                                        ACVP_SHA1, 20);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 2048,
                                        ACVP_SHA224, 24);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 2048,
                                        ACVP_SHA512, 64);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 3072,
                                        ACVP_SHA1, 20);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 3072,
                                        ACVP_SHA224, 24);
  ...
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 3072,
                                        ACVP_SHA512, 64);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 4096,
                                        ACVP_SHA1, 20);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_sigver_set_mod_parm(ctx, ACVP_RSA_SIG_TYPE_PKCS1PSS, 4096,
                                        ACVP_SHA224, 24);
  ...
  rv =
      acvp_cap_rsa_prim_enable(ctx, ACVP_RSA_DECPRIM, &app_rsa_decprim_handler);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  // rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_DECPRIM,
  // ACVP_RSA_PARM_REVISION, ACVP_REVISION_1_0); CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_parm(
      ctx, ACVP_RSA_DECPRIM, ACVP_RSA_PARM_KEY_FORMAT, ACVP_RSA_KEY_FORMAT_CRT);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_DECPRIM,
                                  ACVP_RSA_PARM_PUB_EXP_MODE,
                                  ACVP_RSA_PUB_EXP_MODE_FIXED);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_DECPRIM, ACVP_RSA_PARM_MODULO,
                                  2048);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_DECPRIM, ACVP_RSA_PARM_MODULO,
                                  3072);
  ...
  rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_DECPRIM, ACVP_RSA_PARM_MODULO,
                                  4096);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_exponent(
      ctx, ACVP_RSA_DECPRIM, ACVP_RSA_PARM_FIXED_PUB_EXP_VAL, expo_str);
  CHECK_ENABLE_CAP_RV(rv);
#endif

  /* Enable Signature Primitive */
  rv =
      acvp_cap_rsa_prim_enable(ctx, ACVP_RSA_SIGPRIM, &app_rsa_sigprim_handler);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_parm(
      ctx, ACVP_RSA_SIGPRIM, ACVP_RSA_PARM_KEY_FORMAT, ACVP_RSA_KEY_FORMAT_CRT);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_SIGPRIM,
                                  ACVP_RSA_PARM_PUB_EXP_MODE,
                                  ACVP_RSA_PUB_EXP_MODE_FIXED);
  CHECK_ENABLE_CAP_RV(rv);
  // rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_SIGPRIM,
  // ACVP_RSA_PARM_REVISION, ACVP_REVISION_1_0); CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_rsa_prim_set_parm(ctx, ACVP_RSA_SIGPRIM, ACVP_RSA_PARM_MODULO,
                                  2048);
  CHECK_ENABLE_CAP_RV(rv);
...
#endif
  rv = acvp_cap_rsa_prim_set_exponent(
      ctx, ACVP_RSA_SIGPRIM, ACVP_RSA_PARM_FIXED_PUB_EXP_VAL, expo_str);
  CHECK_ENABLE_CAP_RV(rv);
...
static int enable_ecdsa(ACVP_CTX *ctx) {
  ...
  CHECK_ENABLE_CAP_RV(rv);
#ifndef ACVP_FIPS186_5
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYGEN, ACVP_ECDSA_REVISION,
                               ACVP_REVISION_FIPS186_4);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K233);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K283);
  ...
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_P521);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYGEN, ACVP_ECDSA_SECRET_GEN,
  ...
  CHECK_ENABLE_CAP_RV(rv);
#ifndef ACVP_FIPS186_5
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYVER, ACVP_ECDSA_REVISION,
                               ACVP_REVISION_FIPS186_4);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYVER, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K233);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_KEYVER, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K283);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGGEN, ACVP_ECDSA_COMPONENT_TEST,
                              ACVP_ECDSA_COMPONENT_MODE_BOTH);
  CHECK_ENABLE_CAP_RV(rv);

#ifndef ACVP_FIPS186_5
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGGEN, ACVP_ECDSA_REVISION,
                               ACVP_REVISION_FIPS186_4);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K233);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K283);
  ...
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_P521);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGGEN, ACVP_ECDSA_HASH_ALG,
                               ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGGEN, ACVP_ECDSA_HASH_ALG,
                               ACVP_SHA256);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_DET_ECDSA_SIGGEN,
                               ACVP_ECDSA_COMPONENT_TEST,
                               ACVP_ECDSA_COMPONENT_MODE_NO);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_DET_ECDSA_SIGGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_P224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_DET_ECDSA_SIGGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_P256);
  ...
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_DET_ECDSA_SIGGEN, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_P521);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_DET_ECDSA_SIGGEN, ACVP_ECDSA_HASH_ALG,
                               ACVP_SHA224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_DET_ECDSA_SIGGEN, ACVP_ECDSA_HASH_ALG,
                               ACVP_SHA256);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGVER, ACVP_ECDSA_COMPONENT_TEST,
                              ACVP_ECDSA_COMPONENT_MODE_BOTH);
  CHECK_ENABLE_CAP_RV(rv);
#ifndef ACVP_FIPS186_5
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGVER, ACVP_ECDSA_REVISION,
                               ACVP_REVISION_FIPS186_4);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGVER, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K233);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGVER, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_K283);
  ...
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGVER, ACVP_ECDSA_CURVE,
                               ACVP_EC_CURVE_P192);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGVER, ACVP_ECDSA_HASH_ALG,
                               ACVP_SHA1);
  CHECK_ENABLE_CAP_RV(rv);
#endif
  rv = acvp_cap_ecdsa_set_parm(ctx, ACVP_ECDSA_SIGVER, ACVP_ECDSA_CURVE,
...
static int enable_drbg(ACVP_CTX *ctx) {
  ...
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_1, 0,
                                ACVP_DRBG_ENTROPY_LEN, 128, 64, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_1, 0,
                                ACVP_DRBG_NONCE_LEN, 96, 32, 128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_1, 0,
                                ACVP_DRBG_PERSO_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_1, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_1, 0,
                              ACVP_DRBG_RET_BITS_LEN, 160);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_224, 0,
                                ACVP_DRBG_ENTROPY_LEN, 192, 64, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_224, 0,
                                ACVP_DRBG_NONCE_LEN, 128, 32, 160);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_224, 0,
                                ACVP_DRBG_PERSO_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_224, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_224, 0,
                              ACVP_DRBG_RET_BITS_LEN, 224);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_256, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 64, 320);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_256, 0,
                                ACVP_DRBG_NONCE_LEN, 128, 32, 160);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_256, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_256, 0,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_384, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 64, 320);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_384, 0,
                                ACVP_DRBG_NONCE_LEN, 128, 32, 160);
  ...
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_384, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_384, 0,
                              ACVP_DRBG_RET_BITS_LEN, 384);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 64, 320);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512, 0,
                                ACVP_DRBG_NONCE_LEN, 128, 32, 160);
  ...
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512, 0,
                              ACVP_DRBG_RET_BITS_LEN, 512);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_224, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 64, 320);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_224, 0,
                                ACVP_DRBG_NONCE_LEN, 128, 32, 160);
  ...
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_224, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_224, 0,
                              ACVP_DRBG_RET_BITS_LEN, 224);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_256, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 64, 320);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_256, 0,
                                ACVP_DRBG_NONCE_LEN, 128, 32, 160);
  ...
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_256, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HASHDRBG, ACVP_DRBG_SHA_512_256, 0,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  /* Group number for these should be 0 */
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_1, 0,
                              ACVP_DRBG_PRED_RESIST_ENABLED, 1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_1, 0,
                              ACVP_DRBG_RESEED_ENABLED, 1);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_1, 0,
                              ACVP_DRBG_RET_BITS_LEN, 160);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_1, 0,
                                ACVP_DRBG_ENTROPY_LEN, 160, 32, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_1, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 64);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_1, 0,
                                ACVP_DRBG_PERSO_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_224, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 96);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_224, 0,
                                ACVP_DRBG_PERSO_LEN, 0, 64, 192);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_224, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 0, 192);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_256, 0,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_256, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 64, 512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_256, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_256, 0,
                                ACVP_DRBG_PERSO_LEN, 0, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_384, 0,
                              ACVP_DRBG_RET_BITS_LEN, 384);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_384, 0,
                                ACVP_DRBG_ENTROPY_LEN, 384, 64, 512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_384, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512, 0,
                              ACVP_DRBG_RET_BITS_LEN, 512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512, 0,
                                ACVP_DRBG_ENTROPY_LEN, 512, 64, 1024);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512_224, 0,
                              ACVP_DRBG_RET_BITS_LEN, 224);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512_224, 0,
                                ACVP_DRBG_ENTROPY_LEN, 512, 64, 1024);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512_224, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512_256, 0,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512_256, 0,
                                ACVP_DRBG_ENTROPY_LEN, 512, 64, 1024);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_HMACDRBG, ACVP_DRBG_SHA_512_256, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  ...
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 0,
                                ACVP_DRBG_ENTROPY_LEN, 128, 128, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 0,
                                ACVP_DRBG_PERSO_LEN, 0, 256, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 0,
                                ACVP_DRBG_ADD_IN_LEN, 0, 256, 256);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 1,
                              ACVP_DRBG_DER_FUNC_ENABLED, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 1,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 1,
                                ACVP_DRBG_ENTROPY_LEN, 256, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 1,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 1,
                                ACVP_DRBG_PERSO_LEN, 256, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_128, 1,
                                ACVP_DRBG_ADD_IN_LEN, 256, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 0,
                              ACVP_DRBG_DER_FUNC_ENABLED, 1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 0,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 128, 512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  CHECK_ENABLE_CAP_RV(rv);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 1,
                              ACVP_DRBG_DER_FUNC_ENABLED, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 1,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 1,
                                ACVP_DRBG_ENTROPY_LEN, 320, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 1,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 1,
                                ACVP_DRBG_PERSO_LEN, 320, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_192, 1,
                                ACVP_DRBG_ADD_IN_LEN, 320, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);

  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 0,
                              ACVP_DRBG_DER_FUNC_ENABLED, 1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 0,
                              ACVP_DRBG_PRED_RESIST_ENABLED, 1);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 0,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 0,
                                ACVP_DRBG_ENTROPY_LEN, 256, 128, 512);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 0,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 128);
  ...
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 1,
                              ACVP_DRBG_DER_FUNC_ENABLED, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_parm(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 1,
                              ACVP_DRBG_RET_BITS_LEN, 256);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 1,
                                ACVP_DRBG_ENTROPY_LEN, 384, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 1,
                                ACVP_DRBG_NONCE_LEN, 0, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 1,
                                ACVP_DRBG_PERSO_LEN, 384, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_drbg_set_length(ctx, ACVP_CTRDRBG, ACVP_DRBG_AES_256, 1,
                                ACVP_DRBG_ADD_IN_LEN, 384, 0, 0);
  CHECK_ENABLE_CAP_RV(rv);
end:
  return rv;
}
...
static int enable_safe_primes(ACVP_CTX *ctx) {
  ...
  rv = acvp_cap_safe_primes_enable(ctx, ACVP_SAFE_PRIMES_KEYGEN,
                                   &app_safe_primes_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_set_prereq(ctx, ACVP_SAFE_PRIMES_KEYGEN, ACVP_PREREQ_DRBG,
                           value);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_set_prereq(ctx, ACVP_SAFE_PRIMES_KEYGEN, ACVP_PREREQ_SHA, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_safe_primes_set_parm(ctx, ACVP_SAFE_PRIMES_KEYGEN,
                                     ACVP_SAFE_PRIMES_GENMETH,
  ...
  rv = acvp_cap_safe_primes_enable(ctx, ACVP_SAFE_PRIMES_KEYVER,
                                   &app_safe_primes_handler);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_set_prereq(ctx, ACVP_SAFE_PRIMES_KEYVER, ACVP_PREREQ_DRBG,
                           value);
  CHECK_ENABLE_CAP_RV(rv);
  rv =
      acvp_cap_set_prereq(ctx, ACVP_SAFE_PRIMES_KEYVER, ACVP_PREREQ_SHA, value);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_safe_primes_set_parm(ctx, ACVP_SAFE_PRIMES_KEYVER,
                                     ACVP_SAFE_PRIMES_GENMETH,
                                     ACVP_SAFE_PRIMES_FFDHE2048);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_safe_primes_set_parm(ctx, ACVP_SAFE_PRIMES_KEYVER,
...
static int enable_lms(ACVP_CTX *ctx) {
...
#if 0 /* An example of how an LMS and LMOTS mode can be registered as a        \
         compatible pair exclusively */
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_SIGVER, ACVP_LMS_PARAM_LMS_MODE,
  ...
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_SIGVER, ACVP_LMS_PARAM_LMOTS_MODE,
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_KEYGEN, ACVP_LMS_PARAM_LMS_MODE,
                             ACVP_LMS_MODE_SHA256_M24_H5);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_KEYGEN, ACVP_LMS_PARAM_LMS_MODE,
                             ACVP_LMS_MODE_SHA256_M24_H10);
  ...
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_KEYGEN, ACVP_LMS_PARAM_LMOTS_MODE,
                             ACVP_LMOTS_MODE_SHA256_N24_W1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_KEYGEN, ACVP_LMS_PARAM_LMOTS_MODE,
                             ACVP_LMOTS_MODE_SHA256_N24_W2);
  ...
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_SIGGEN, ACVP_LMS_PARAM_LMS_MODE,
                             ACVP_LMS_MODE_SHA256_M24_H5);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_SIGGEN, ACVP_LMS_PARAM_LMS_MODE,
                             ACVP_LMS_MODE_SHA256_M24_H10);
  ...
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_SIGGEN, ACVP_LMS_PARAM_LMOTS_MODE,
                             ACVP_LMOTS_MODE_SHA256_N24_W1);
  CHECK_ENABLE_CAP_RV(rv);
  rv = acvp_cap_lms_set_parm(ctx, ACVP_LMS_SIGGEN, ACVP_LMS_PARAM_LMOTS_MODE,
                             ACVP_LMOTS_MODE_SHA256_N24_W2);
...
ACVP_RESULT acvp_app_run_vector_test_file(const char *path, const char *output,
                                          ACVP_LOG_LVL lvl,
                                          ACVP_RESULT (*logger)(char *)) {
  ACVP_RESULT rv = ACVP_SUCCESS;
  ACVP_CTX *ctx = NULL;
