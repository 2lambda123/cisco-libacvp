...
#define ACVP_BIT2BYTE(x)                                                       \
  ((x + 7) >>                                                                  \
   3) /**< Convert bit length (x, of type integer) into byte length */

#define ACVP_ALG_MAX ACVP_CIPHER_END - 1 /* Used by alg_tbl[] */

#define ACVP_CAP_MAX                                                           \
  ACVP_ALG_MAX * 2 /* Arbitrary limit to the number of capability objects that \
...
#define ACVP_HASH_MSG_STR_MAX                                                  \
  (ACVP_HASH_MSG_BIT_MAX >> 2) /**< 16384 characters */
...
#define ACVP_SHAKE_MSG_BYTE_MAX                                                \
  (ACVP_SHAKE_MSG_BIT_MAX >> 3) /**< 16384 bytes */
...
#define ACVP_KDF135_SSH_STR_IN_MAX                                             \
  4096 /**< 4096 characters, needs to accomodate large shared_secret (K) */
...
typedef struct acvp_curve_hash_compatibility_list_t {
  ACVP_EC_CURVE curve;
  ACVP_HASH_ALG algs[ACVP_HASH_ALG_MAX]; // flags for all supported hash algs
                                         // for curve, use arr so we can memcmp
...
typedef struct acvp_ecdsa_capability_t {
  ...
  // For backwards compatibility, this contains hash algs that will be used with
  // ALL registered curves (HASH_ALG = array index)
...
typedef struct acvp_kda_twostep_capability_t {
  ...
  ACVP_KDF108_CAP kdf_params; /* All of the KDF108 params get stored in here to
...
typedef struct acvp_oe_dependencies_t {
  ACVP_DEPENDENCY *deps[LIBACVP_DEPENDENCIES_MAX]; /* Array to pointers of
                                                      linked dependencies */
  unsigned int count;
  ACVP_RESOURCE_STATUS status; /**< PARTIAL indicates that at least one of the
                                  linked Dependencies does not exist. INCOMPLETE
                                  indicates all of the 'url' are missing */
...
typedef struct acvp_fips_t {
  ...
  int metadata_loaded; /* Flag indicating whether the metadata necessary for
                         a FIPS validation was successfully loaded into memory.
                         1 for yes */
...
struct acvp_ctx_t {
  ...
  char *session_file_path; /* String containing the path of the testSession file
                              after it is created when applicable */
  ...
  int use_json;  /* flag to indicate a JSON file is being used for registration
                  */
  int is_sample; /* flag to idicate that we are requesting sample vector
                    responses */
  ...
  int vector_req; /* flag to indicate we are storing vector request JSON in a
                     file */
  int vector_rsp; /* flag to indicate we are storing vector responses JSON in a
                     file */
  ...
  char *jwt_token; /* access_token provided by server for authenticating REST
                      calls */
  char *tmp_jwt; /* access_token provided by server for authenticating a single
                    REST call */
  ...
  /* Maintain a count of the number of registered vector sets so we can evaluate
   * cost. This can be >= caps_list size */
  int vs_count;
  ...
  int post_size_constraint; /**< The number of bytes that the body of an HTTP
                               POST may contain without requiring the use of the
                               /large endpoint. If the POST body is larger than
                               this value, then use of the /large endpoint is
                               necessary */
};
