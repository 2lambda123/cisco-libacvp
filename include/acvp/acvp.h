 ...
 * may want to intervene, but are not neccessarily issues and running will
...
typedef enum acvp_result {
  ...
  ACVP_CLEANUP_FAIL,   /**< Failure when cleaning up (e.g. freeing memory) after
                          operations */
  ACVP_KAT_DOWNLOAD_RETRY, /**< Does not neccessarily indicate an error, but
  ...
  ACVP_CRYPTO_MODULE_FAIL, /**< A non-zero return code was provided by the
                              application callback for test case processin; this
                              should indicate that the application failed to
                              process the test case*/
  ACVP_NO_CAP, /**< A registered capability object for the given algorithm does
                  not exist. This usually means an operation is being requested
                  for an algorithm that is not yet registered */
  ...
  ACVP_TC_INVALID_DATA, /**< Test case JSON is formatted properly, but the data
                           is bad, does not match the registration, or does not
                           match the spec */
  ACVP_DATA_TOO_LARGE,  /**< The given parameter larger than the library allows.
                           This can apply to strings,  server responses, files,
                           etc */
  ...
  ACVP_JWT_MISSING, /**< A JSON web token is missing from a file or from memory
                       but was expected */
  ACVP_JWT_EXPIRED, /**< The provided JWT was not accepted by the server because
                       it is expired */
 ...
 * library determines which ones revisions are allowed for which algorihms.
 */
...
/**
 * The following enumerators are used to track the capabiltiies that the
 ...
 * @brief These are the algorithm direction suppported by libacvp. These are
 * used in conjunction with ACVP_SYM_CIPH when registering the crypto module
...
typedef struct acvp_hash_tc_t {
  ...
  unsigned int
      xof_bit_len; /**< XOF (extendable output format) length
...
typedef struct acvp_kdf135_x942_tc_t {
  ...
  unsigned char
      *oid; /**< OID is a unique identifier (bit string) of the cipher using the
             * DKM. These are fixed, specific values in ANSI X9.42 */
  unsigned char *zz;
...
typedef struct acvp_kdf_tls12_tc_t {
  ...
  unsigned char *c_rnd;
  unsigned char
      *msecret;          /**< The resulting data calculated for the test case */
...
typedef struct acvp_kdf_tls13_tc_t {
  ...
  unsigned char
      *dhe; /**< diffie hellman secret, populated for DHE or PSK-DHE */
...
typedef struct acvp_kmac_tc_t {
  ...
  unsigned char *msg;
  unsigned char *mac; /**< The resulting digest calculated for the test case, or
                         provided when verifying */
  unsigned char *key;
...
typedef struct acvp_kda_onestep_tc_t {
  ...
  // Incrementing through the array, each element represents a pattern candidate
  // until we reach a 0
...
typedef struct acvp_drbg_tc_t {
  ...
  unsigned int
      perso_string_len;     /**< Personalization String length (in bytes) */
 ...
 * capability length-based paramter.
 *
 *        This function should be used to register a length-based parameter for
 * a DRBG capability. An example would be entropy, nonce, perso where a minimum,
 * step, and maximum can be specified.
 *
 * @param ctx Pointer to ACVP_CTX that was previously created by calling
 * acvp_create_test_session.
 * @param cipher ACVP_CIPHER enum value identifying the crypto capability.
 ...
 * must be used in a linear fashion (group 0 must be defined before you can
 * define group 1, group 1 before group 2, etc)
 * @param param ACVP_DRBG_PARM enum value specifying paramter. An example would
...
/**
 * @brief acvp_free_test_session() releases the memory associated withan
 ...
 * capabilities registered thus far will be returrned.
 *
 ...
 * acvp_create_test_session.
 * @param request_filename File containing the session info created by libacvp
 * @param save_filename OPTIONAL arugment indicated a file the server response
