 ...
 */
static ACVP_RESULT acvp_rsa_output_tc(ACVP_CTX *ctx, ACVP_RSA_KEYGEN_TC *stc,
                                      JSON_Object *tc_rsp) {
  ACVP_RESULT rv = ACVP_SUCCESS;
  char *tmp = NULL;

  if (stc->rand_pq == ACVP_RSA_KEYGEN_B33 &&
      stc->test_type == ACVP_RSA_TESTTYPE_KAT) {
  ...
  if (stc->info_gen_by_server) {
    if (stc->rand_pq == ACVP_RSA_KEYGEN_B33 ||
      ...
      json_object_set_string(tc_rsp, "primeResult",
                             (const char *)stc->prime_result);
    }
  } else {
    if (!(stc->rand_pq == ACVP_RSA_KEYGEN_B33)) {
      rv = acvp_bin_to_hexstr(stc->seed, stc->seed_len, tmp,
...
static ACVP_RESULT acvp_rsa_keygen_init_tc(
    ACVP_CTX *ctx, ACVP_RSA_KEYGEN_TC *stc, unsigned int tc_id,
    ACVP_RSA_TESTTYPE test_type, int info_gen_by_server, ACVP_HASH_ALG hash_alg,
    ACVP_RSA_KEY_FORMAT key_format, ACVP_RSA_PUB_EXP_MODE pub_exp_mode,
    int modulo, ACVP_RSA_PRIME_TEST_TYPE prime_test, int rand_pq, const char *e,
    const char *p, const char *q, const char *xp, const char *xp1,
    const char *xp2, const char *xq, const char *xq1, const char *xq2,
    const char *seed, int seed_len, int bitlen1, int bitlen2, int bitlen3,
    int bitlen4) {
  memzero_s(stc, sizeof(ACVP_RSA_KEYGEN_TC));
  ACVP_RESULT rv = ACVP_SUCCESS;
  ...
  if (rand_pq == ACVP_RSA_KEYGEN_B36) {
    ...
    rv = acvp_hexstr_to_bin(xp1, stc->xp1, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->xp1_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (xp1)");
      return rv;
    }
    rv = acvp_hexstr_to_bin(xp2, stc->xp2, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->xp2_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (xp2)");
      return rv;
    ...
    rv = acvp_hexstr_to_bin(xq1, stc->xq1, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->xq1_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (xq1)");
      return rv;
    }
    rv = acvp_hexstr_to_bin(xq2, stc->xq2, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->xq2_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (xq2)");
      return rv;
...
static ACVP_RSA_TESTTYPE read_test_type(const char *str) {
  int diff = 1;
  strcmp_s(ACVP_TESTTYPE_STR_KAT, sizeof(ACVP_TESTTYPE_STR_KAT) - 1, str,
           &diff);
  if (!diff)
    return ACVP_RSA_TESTTYPE_KAT;
  strcmp_s(ACVP_TESTTYPE_STR_AFT, sizeof(ACVP_TESTTYPE_STR_AFT) - 1, str,
           &diff);
  if (!diff)
    return ACVP_RSA_TESTTYPE_AFT;
  strcmp_s(ACVP_TESTTYPE_STR_GDT, sizeof(ACVP_TESTTYPE_STR_GDT) - 1, str,
           &diff);
  if (!diff)
    return ACVP_RSA_TESTTYPE_GDT;
...
static ACVP_RSA_KEY_FORMAT read_key_format(const char *str) {
  ...
  strcmp_s(ACVP_RSA_KEY_FORMAT_STD_STR, ACVP_RSA_KEY_FORMAT_STD_STR_LEN, str,
           &diff);
  if (!diff)
    return ACVP_RSA_KEY_FORMAT_STANDARD;

  strcmp_s(ACVP_RSA_KEY_FORMAT_CRT_STR, ACVP_RSA_KEY_FORMAT_CRT_STR_LEN, str,
           &diff);
  if (!diff)
    return ACVP_RSA_KEY_FORMAT_CRT;
...
static ACVP_RSA_PRIME_TEST_TYPE read_prime_test_type(const char *str) {
  ...
  strcmp_s(ACVP_RSA_PRIME_TEST_TBLC2_STR, ACVP_RSA_PRIME_TEST_TBLC2_STR_LEN,
           str, &diff);
  if (!diff)
    return ACVP_RSA_PRIME_TEST_TBLC2;

  strcmp_s(ACVP_RSA_PRIME_TEST_TBLC3_STR, ACVP_RSA_PRIME_TEST_TBLC3_STR_LEN,
           str, &diff);
  if (!diff)
    return ACVP_RSA_PRIME_TEST_TBLC3;
...
ACVP_RESULT acvp_rsa_keygen_kat_handler(ACVP_CTX *ctx, JSON_Object *obj) {
  ...
  const char *e_str = NULL, *p_str = NULL, *q_str = NULL, *xp_str = NULL,
             *xp1_str = NULL, *xp2_str = NULL, *xq_str = NULL, *xq1_str = NULL,
             *xq2_str = NULL, *alg_str = NULL, *mode_str = NULL,
             *hash_alg_str = NULL, *seed = NULL, *pub_exp_mode_str = NULL,
             *key_format_str = NULL, *rand_pq_str = NULL,
             *prime_test_str = NULL, *test_type_str = NULL;
  ...
  for (i = 0; i < g_cnt; i++) {
    ...
    info_gen_by_server =
        json_object_get_boolean(groupobj, "infoGeneratedByServer");
    ...
    if (rand_pq == ACVP_RSA_KEYGEN_B33 || rand_pq == ACVP_RSA_KEYGEN_B35 ||
    ...
    if (rand_pq == ACVP_RSA_KEYGEN_B32 || rand_pq == ACVP_RSA_KEYGEN_B34 ||
    ...
    ACVP_LOG_VERBOSE("  infoGenByServer: %s",
                     info_gen_by_server ? "true" : "false");
    ...
    for (j = 0; j < t_cnt; j++) {
      ...
      if (info_gen_by_server && pub_exp_mode == ACVP_RSA_PUB_EXP_MODE_RANDOM) {
        ...
        if (strnlen_s(e_str, ACVP_RSA_EXP_LEN_MAX + 1) > ACVP_RSA_EXP_LEN_MAX) {
          ACVP_LOG_ERR("'e' too long, max allowed=(%d)", ACVP_RSA_EXP_LEN_MAX);
          rv = ACVP_INVALID_ARG;
          json_value_free(r_tval);
          goto err;
        }
      ...
      if (info_gen_by_server) {
        ...
        if (rand_pq >= ACVP_RSA_KEYGEN_B34) {
          ...
          if (count != 4) {
            ACVP_LOG_ERR(
                "Server JSON 'bitlens' list count is (%u). Expected (%u)",
                count, 4);
            rv = ACVP_INVALID_ARG;
            json_value_free(r_tval);
            goto err;
      ...
      if (rand_pq == ACVP_RSA_KEYGEN_B33 &&
          test_type == ACVP_RSA_TESTTYPE_KAT) {
        // E
        e_str = json_object_get_string(testobj, "e");
        if (!e_str) {
          ACVP_LOG_ERR("Server JSON missing 'e'");
        ...
        if (strnlen_s(p_str, ACVP_RSA_EXP_LEN_MAX + 1) > ACVP_RSA_EXP_LEN_MAX) {
          ACVP_LOG_ERR("'p' too long, max allowed=(%d)", ACVP_RSA_EXP_LEN_MAX);
          rv = ACVP_INVALID_ARG;
          json_value_free(r_tval);
          goto err;
        }
      ...
      rv = acvp_rsa_keygen_init_tc(
          ctx, &stc, tc_id, test_type, info_gen_by_server, hash_alg, key_format,
          pub_exp_mode, mod, prime_test, rand_pq, e_str, p_str, q_str, xp_str,
          xp1_str, xp2_str, xq_str, xq1_str, xq2_str, seed, seed_len, bitlen1,
          bitlen2, bitlen3, bitlen4);
