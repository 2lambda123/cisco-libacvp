...
void acvp_log_msg(ACVP_CTX *ctx, ACVP_LOG_LVL level, const char *func, int line,
  ...
  if (ctx->test_progress_cb && (ctx->log_lvl >= level)) {
    ...
    ret =
        vsnprintf(tmp + iter, ACVP_LOG_MAX_MSG_LEN + 1 - iter, fmt, arguments);
    if (ret < 0 || ret >= ACVP_LOG_MAX_MSG_LEN + 1 - iter) {
      memcpy_s(tmp + ACVP_LOG_MAX_MSG_LEN - ACVP_LOG_TRUNCATED_STR_LEN,
               ACVP_LOG_TRUNCATED_STR_LEN, ACVP_LOG_TRUNCATED_STR,
...
/*
 * Sometimes there is a need for line separation in the logs, but we still
 * prefer for the app handler to deal with it instead of making assumptions
 * about output
 */
...
/**
 * @brief This function returns the default revision of an algorithm given a
 * ACVP_CIPHER value.
 *
 ...
 *
 *        Note that this should not be used when the user registers an
 * alternative revision for a cipher.
 *
...
/**
 * This table maintains a list of strings for revisions that can be registered
 * as capabilities.
 */
...
/**
 * this function returns the string used in registration for an alternative
 * revision registered by a user.
 */
...
/**
 * This function returns the enum for a given alternative revision string, or 0
 * if not found
 */
...
ACVP_CIPHER acvp_lookup_cipher_index(const char *algorithm) {
  ...
  for (i = 0; i < ACVP_ALG_MAX; i++) {
    ...
    strcmp_s(alg_tbl[i].name, ACVP_ALG_NAME_MAX, algorithm, &diff);
...
ACVP_RSA_PUB_EXP_MODE acvp_lookup_rsa_pub_exp_mode(const char *str) {
  ...
  strcmp_s(ACVP_RSA_PUB_EXP_MODE_FIXED_STR, ACVP_RSA_PUB_EXP_MODE_FIXED_STR_LEN,
           str, &diff);
  if (!diff)
    return ACVP_RSA_PUB_EXP_MODE_FIXED;

  strcmp_s(ACVP_RSA_PUB_EXP_MODE_RANDOM_STR,
           ACVP_RSA_PUB_EXP_MODE_RANDOM_STR_LEN, str, &diff);
  if (!diff)
    return ACVP_RSA_PUB_EXP_MODE_RANDOM;
...
int acvp_lookup_rsa_randpq_index(const char *value) {
  ...
  if (!diff)
    return ACVP_RSA_KEYGEN_B32;

  strcmp_s("B.3.3", 5, value, &diff);
  if (!diff)
    return ACVP_RSA_KEYGEN_B33;

  strcmp_s("B.3.4", 5, value, &diff);
  if (!diff)
    return ACVP_RSA_KEYGEN_B34;

  strcmp_s("B.3.5", 5, value, &diff);
  if (!diff)
    return ACVP_RSA_KEYGEN_B35;

  strcmp_s("B.3.6", 5, value, &diff);
  if (!diff)
    return ACVP_RSA_KEYGEN_B36;
...
ACVP_DRBG_MODE acvp_lookup_drbg_mode_index(const char *mode) {
  ...
  for (i = 0; i < drbg_mode_tbl_length; i++) {
    int diff = 0;
    strcmp_s(drbg_mode_tbl[i].name, DRBG_MODE_NAME_MAX, mode, &diff);
...
ACVP_HASH_ALG acvp_lookup_hash_alg(const char *name) {
  ...
  for (i = 0; i < hash_alg_tbl_length; i++) {
    ...
    strcmp_s(hash_alg_tbl[i].name, HASH_ALG_NAME_MAX, name, &diff);
...
static struct acvp_ec_curve_info ec_curve_tbl[] = {
    {ACVP_EC_CURVE_P224, "P-224"}, {ACVP_EC_CURVE_P256, "P-256"},
    {ACVP_EC_CURVE_P384, "P-384"}, {ACVP_EC_CURVE_P521, "P-521"},
    {ACVP_EC_CURVE_B233, "B-233"}, {ACVP_EC_CURVE_B283, "B-283"},
    {ACVP_EC_CURVE_B409, "B-409"}, {ACVP_EC_CURVE_B571, "B-571"},
    {ACVP_EC_CURVE_K233, "K-233"}, {ACVP_EC_CURVE_K283, "K-283"},
    {ACVP_EC_CURVE_K409, "K-409"}, {ACVP_EC_CURVE_K571, "K-571"}};
static int ec_curve_tbl_length =
    sizeof(ec_curve_tbl) / sizeof(struct acvp_ec_curve_info);

/*
 * Local table for matching ACVP_EC_CURVE to name string and vice versa.
 * Containes "deprecated" curves (still allowed for ECDSA_KEYVER and
 * ECDSA_SIGVER).
 */
...
ACVP_EC_CURVE acvp_lookup_ec_curve(ACVP_CIPHER cipher, const char *name) {
  ...
  for (i = 0; i < ec_curve_tbl_length; i++) {
    ...
    strcmp_s(ec_curve_tbl[i].name, EC_CURVE_NAME_MAX, name, &diff);
  ...
  if (cipher == ACVP_ECDSA_KEYVER || cipher == ACVP_ECDSA_SIGVER) {
    ...
    for (i = 0; i < ec_curve_depr_tbl_length; i++) {
      ...
      strcmp_s(ec_curve_depr_tbl[i].name, EC_CURVE_NAME_MAX, name, &diff);
...
static int acvp_aux_function_tbl_len =
    sizeof(acvp_aux_function_tbl) / sizeof(struct acvp_function_info);
...
ACVP_CIPHER acvp_lookup_aux_function_alg_tbl(const char *str) {
  int diff = 1, i = 0;
  for (i = 0; i < acvp_aux_function_tbl_len; i++) {
    strcmp_s(acvp_aux_function_tbl[i].name,
             strnlen_s(acvp_aux_function_tbl[i].name, ACVP_ALG_NAME_MAX), str,
             &diff);
    if (!diff) {
...
static int lms_mode_tbl_len =
    sizeof(lms_mode_tbl) / sizeof(struct acvp_enum_string_pair);

ACVP_LMS_MODE acvp_lookup_lms_mode(const char *str) {
  int diff = 1, i = 0;
  for (i = 0; i < lms_mode_tbl_len; i++) {
    strcmp_s(lms_mode_tbl[i].string,
             strnlen_s(lms_mode_tbl[i].string, ACVP_LMS_MODE_STR_MAX), str,
             &diff);
    if (!diff) {
...
static int lmots_mode_tbl_len =
    sizeof(lmots_mode_tbl) / sizeof(struct acvp_enum_string_pair);

ACVP_LMOTS_MODE acvp_lookup_lmots_mode(const char *str) {
  int diff = 1, i = 0;
  for (i = 0; i < lmots_mode_tbl_len; i++) {
    strcmp_s(lmots_mode_tbl[i].string,
             strnlen_s(lmots_mode_tbl[i].string, ACVP_LMS_MODE_STR_MAX), str,
             &diff);
    if (!diff) {
...
/* This seems too small to dictate having its own table/function, but future
 * expandability may be useful */
static struct acvp_enum_string_pair rsa_key_format_tbl[] = {
    {ACVP_RSA_KEY_FORMAT_STANDARD, "standard"},
    {ACVP_RSA_KEY_FORMAT_CRT, "crt"}};

static int rsa_key_format_tbl_len =
    sizeof(rsa_key_format_tbl) / sizeof(struct acvp_enum_string_pair);
...
ACVP_RESULT acvp_hexstr_to_bin(const char *src, unsigned char *dest,
  ...
  if (!is_odd) {
    while (*src && src[1]) {
      byte_a = acvp_char_to_int((char)*src)
               << 4; /* Shift to left half of byte */
  ...
  if (converted_len)
    *converted_len = length_converted;
  return ACVP_SUCCESS;
}
...
const char *acvp_lookup_error_string(ACVP_RESULT rv) {
  ...
  struct acvp_result_desc_t error_desc_tbl[ACVP_RESULT_MAX - 1] = {
      ...
      {ACVP_CRYPTO_MODULE_FAIL,
       "Error from crypto module processing a vector set"},
...
/**
 * @brief Determine if the given \p string fits within the \p max_allowed
 * length.
 *
...
/**
 * Simple utility function to add an entry to a SL list. if the list is NULL, it
 * is created with the given entry being the first one.
 */
...
/**
 * Simple utility function to add an entry to a param list. if the list is NULL,
 * it is created with the given entry being the first one.
 */
...
/**
 * Simple utility function to add a entry to a name list. If the list is NULL,
 * it is created with the given entry being the first one. Note the string is
 * REFERENCED, not copied. This function should be able to accomdate the removal
 * of names from the list if needed in the future; if a name is removed from the
 * list but its node remains (with a NULL value) then the given string will be
 * added to the "dummy" node
 */
