 ...
 */
static ACVP_RESULT acvp_rsa_decprim_output_tc_rev_1(ACVP_CTX *ctx,
...
static ACVP_RESULT acvp_rsa_decprim_output_tc_rev_56br2(ACVP_CTX *ctx,
                                                        ACVP_RSA_PRIM_TC *stc,
                                                        JSON_Object *tc_rsp) {
  ACVP_RESULT rv = ACVP_SUCCESS;
  char *tmp = NULL;
...
static ACVP_RESULT acvp_rsa_sigprim_output_tc(ACVP_CTX *ctx,
                                              ACVP_RSA_PRIM_TC *stc,
                                              JSON_Object *tc_rsp) {
  ACVP_RESULT rv = ACVP_SUCCESS;
  char *tmp = NULL;
  ...
  if (stc->disposition) {
    rv = acvp_bin_to_hexstr(stc->signature, stc->sig_len, tmp,
...
static ACVP_RESULT
acvp_rsa_decprim_init_tc_rev_1(ACVP_CTX *ctx, ACVP_RSA_PRIM_TC *stc, int modulo,
                               int deferred, int pass, int fail,
                               const char *cipher, int cipher_len) {
  ...
  rv = acvp_hexstr_to_bin(cipher, stc->cipher, ACVP_RSA_EXP_BYTE_MAX,
                          &(stc->cipher_len));
  if (rv != ACVP_SUCCESS) {
...
static ACVP_RESULT acvp_rsa_decprim_init_tc_rev_56br2(
    ACVP_CTX *ctx, ACVP_RSA_PRIM_TC *stc, ACVP_RSA_PUB_EXP_MODE exp_mode,
    int modulo, ACVP_RSA_KEY_FORMAT format, const char *d, const char *e,
    const char *n, const char *p, const char *q, const char *dmp1,
    const char *dmq1, const char *iqmp, const char *ct) {
  ACVP_RESULT rv = ACVP_SUCCESS;
  ...
  rv = acvp_hexstr_to_bin(ct, stc->cipher, ACVP_RSA_EXP_BYTE_MAX,
                          &(stc->cipher_len));
  if (rv != ACVP_SUCCESS) {
    ACVP_LOG_ERR("Hex conversion failure (cipher)");
    return rv;
  ...
  if (dmp1) {
    ...
    rv = acvp_hexstr_to_bin(dmp1, stc->dmp1, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->dmp1_len));
    if (rv != ACVP_SUCCESS) {
  ...
  if (dmq1) {
    ...
    rv = acvp_hexstr_to_bin(dmq1, stc->dmq1, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->dmq1_len));
    if (rv != ACVP_SUCCESS) {
  ...
  if (iqmp) {
    ...
    rv = acvp_hexstr_to_bin(iqmp, stc->iqmp, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->iqmp_len));
    if (rv != ACVP_SUCCESS) {
...
static ACVP_RESULT acvp_rsa_sigprim_init_tc(
    ACVP_CTX *ctx, ACVP_RSA_PRIM_TC *stc, unsigned int modulo,
    unsigned int keyformat, const char *d_str, const char *e_str,
    const char *n_str, const char *p_str, const char *q_str,
    const char *dmp1_str, const char *dmq1_str, const char *iqmp_str,
    const char *msg) {
  ...
  if (d_str) {
    ...
    rv =
        acvp_hexstr_to_bin(d_str, stc->d, ACVP_RSA_EXP_BYTE_MAX, &(stc->d_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (d)");
      return rv;
    }
  ...
  if (p_str) {
    ...
    rv =
        acvp_hexstr_to_bin(p_str, stc->p, ACVP_RSA_EXP_BYTE_MAX, &(stc->p_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (p)");
      return rv;
    }
  ...
  if (q_str) {
    ...
    rv =
        acvp_hexstr_to_bin(q_str, stc->q, ACVP_RSA_EXP_BYTE_MAX, &(stc->q_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (q");
      return rv;
    }
  ...
  if (dmp1_str) {
    ...
    rv = acvp_hexstr_to_bin(dmp1_str, stc->dmp1, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->dmp1_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (dmp1)");
      return rv;
  ...
  if (dmq1_str) {
    ...
    rv = acvp_hexstr_to_bin(dmq1_str, stc->dmq1, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->dmq1_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (dmq1)");
      return rv;
  ...
  if (iqmp_str) {
    ...
    rv = acvp_hexstr_to_bin(iqmp_str, stc->iqmp, ACVP_RSA_EXP_BYTE_MAX,
                            &(stc->iqmp_len));
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR("Hex conversion failure (iqmp)");
      return rv;
  ...
  rv =
      acvp_hexstr_to_bin(msg, stc->msg, ACVP_RSA_EXP_BYTE_MAX, &(stc->msg_len));
  if (rv != ACVP_SUCCESS) {
...
ACVP_RSA_KEY_FORMAT acvp_rsa_get_key_format(const char *key_format) {
  ...
  if (!diff)
    keyformat = ACVP_RSA_KEY_FORMAT_STANDARD;

  strcmp_s("crt", 3, key_format, &diff);
  if (!diff)
    keyformat = ACVP_RSA_KEY_FORMAT_CRT;
...
ACVP_RESULT acvp_rsa_decprim_kat_handler(ACVP_CTX *ctx, JSON_Object *obj) {
  ...
  JSON_Array *r_tarr = NULL, *r_garr = NULL,
             *r_carr = NULL; /* Response testarray, grouparray */
  JSON_Value *r_tval = NULL, *r_gval = NULL,
             *r_cval = NULL; /* Response testval, groupval */
  JSON_Object *r_tobj = NULL, *r_gobj = NULL,
              *r_cobj = NULL; /* Response testobj, groupobj */
  ACVP_CAPS_LIST *cap;
  ACVP_RSA_PRIM_TC stc;
  ACVP_TEST_CASE tc;
  ACVP_RESULT rv;
  ...
  const char *alg_str = NULL, *mode_str = NULL, *cipher = NULL, *rev_str = NULL,
             *key_format = NULL, *pub_exp_mode_str = NULL, *e_str = NULL,
             *n_str = NULL, *d_str = NULL, *p_str = NULL, *q_str = NULL,
   ...
   */
  rv = acvp_setup_json_rsp_group(&ctx, &reg_arry_val, &r_vs_val, &r_vs, alg_str,
                                 &r_garr);
  if (rv != ACVP_SUCCESS) {
  ...
  for (i = 0; i < g_cnt; i++) {
    ...
    if (old_rev) {
      ...
      /* Even if fixed, we don't need to parse the pubExp value, its included in
       * each test case as e */
    ...
    for (j = 0; j < t_cnt; j++) {
      ...
      if (old_rev) {
        ...
        for (c = 0; c < c_cnt; c++) {
          ...
          rv = acvp_rsa_decprim_init_tc_rev_1(ctx, &stc, mod, deferred, pass,
          ...
          if (rv == ACVP_SUCCESS) {
            ...
            do {
              ...
              ACVP_LOG_INFO("Looping on fail/pass %d/%d %d/%d", fail, stc.fail,
                            pass, stc.pass);
        ...
        if ((strnlen_s(e_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX) ||
            (strnlen_s(n_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX) ||
            (strnlen_s(p_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX) ||
            (strnlen_s(q_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX) ||
            (strnlen_s(d_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX)) {
        ...
        if (keyformat == ACVP_RSA_KEY_FORMAT_CRT) {
          ...
          if ((strnlen_s(dmp1_str, ACVP_RSA_EXP_LEN_MAX + 1) >
               ACVP_RSA_EXP_LEN_MAX) ||
              (strnlen_s(dmq1_str, ACVP_RSA_EXP_LEN_MAX + 1) >
               ACVP_RSA_EXP_LEN_MAX) ||
              (strnlen_s(iqmp_str, ACVP_RSA_EXP_LEN_MAX + 1) >
               ACVP_RSA_EXP_LEN_MAX)) {
            ACVP_LOG_ERR(
                "server provided p/q/dmp1/dmq1/iqmp of invalid length");
            rv = ACVP_INVALID_ARG;
            json_value_free(r_tval);
            goto err;
          }
        ...
        rv = acvp_rsa_decprim_init_tc_rev_56br2(
            ctx, &stc, keyformat, mod, keyformat, d_str, e_str, n_str, p_str,
...
ACVP_RESULT acvp_rsa_sigprim_kat_handler(ACVP_CTX *ctx, JSON_Object *obj) {
  ...
  JSON_Value *r_vs_val = NULL;
  JSON_Object *r_vs = NULL;
  JSON_Array *r_tarr = NULL,
             *r_garr = NULL; /* Response testarray, grouparray */
  ...
  const char *mode_str;
  const char *msg;
  const char *e_str = NULL, *n_str = NULL, *d_str = NULL, *p_str = NULL,
             *q_str = NULL, *dmp1_str = NULL, *dmq1_str = NULL,
             *iqmp_str = NULL, *rev_str = NULL;
  const char *alg_str;
    ...
    for (j = 0; j < t_cnt; j++) {
      ...
      if (keyformat == ACVP_RSA_KEY_FORMAT_CRT) {
        ...
        if ((strnlen_s(p_str, ACVP_RSA_EXP_LEN_MAX + 1) >
            ...
            (strnlen_s(q_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX) ||
            (strnlen_s(dmp1_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX) ||
            (strnlen_s(dmq1_str, ACVP_RSA_EXP_LEN_MAX + 1) >
             ACVP_RSA_EXP_LEN_MAX) ||
            (strnlen_s(iqmp_str, ACVP_RSA_EXP_LEN_MAX + 1) >
      ...
      rv = acvp_rsa_sigprim_init_tc(ctx, &stc, mod, keyformat, d_str, e_str,
                                    n_str, p_str, q_str, dmp1_str, dmq1_str,
                                    iqmp_str, msg);
