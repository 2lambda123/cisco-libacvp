...
acvp_cap_list_append(ACVP_CTX *ctx, ACVP_CAP_TYPE type, ACVP_CIPHER cipher,
  ...
  /* Assume here one cap = one vector set; for special cases we will handle
   * those as the parameter is set */
  ctx->vs_count++;
  return ACVP_SUCCESS;
...
static ACVP_RESULT acvp_validate_kdf108_param_value(ACVP_CTX *ctx,
  ...
  case ACVP_KDF108_KDF_MODE:
    ACVP_LOG_ERR("No need to explicity enable mode string. It is set implicity "
                 "as params are added to a mode.");
    break;
...
static ACVP_RESULT acvp_validate_prereq_val(ACVP_CIPHER cipher,
  ...
  case ACVP_CMAC_AES:
  case ACVP_CMAC_TDES:
    if (pre_req == ACVP_PREREQ_AES || pre_req == ACVP_PREREQ_SHA ||
 ...
 * values will be phsed out slowly where applicable. For now, allow either set
...
ACVP_RESULT acvp_cap_sym_cipher_set_domain(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  case ACVP_SYM_CIPH_DOMAIN_IVLEN:
    if (symcap->ivlen) {
      ACVP_LOG_ERR(
          "ivLen already defined using acvp_sym_cipher_set_parm. Please set "
          "ivLen using only one function "
    ...
    if (rv != ACVP_SUCCESS) {
      ACVP_LOG_ERR(
          "Unable to validate given domain value (cipher=%d, param=%d)", cipher,
          parm);
      return ACVP_INVALID_ARG;
    }
  ...
  case ACVP_SYM_CIPH_DOMAIN_PTLEN:
    if (symcap->ptlen) {
      ACVP_LOG_ERR(
          "ptLen already defined using acvp_sym_cipher_set_parm. Please set "
          "ptLen using only one function "
  ...
  case ACVP_SYM_CIPH_DOMAIN_AADLEN:
    if (symcap->aadlen) {
      ACVP_LOG_ERR(
          "aadLen already defined using acvp_sym_cipher_set_parm. Please set "
          "aadLen using only one function "
  ...
  case ACVP_SYM_CIPH_DOMAIN_DULEN:
    if (symcap->dulen_matches_paylen) {
      ACVP_LOG_ERR("ACVP_SYM_CIPH_DOMAIN_DULEN can only be set if "
                   "ACVP_SYM_CIPH_PARM_DULEN_MATCHES_PAYLOADLEN is already set "
                   "to 0 (false)");
      return ACVP_INVALID_ARG;
    }
...
ACVP_RESULT acvp_cap_sym_cipher_set_parm(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  case ACVP_SYM_CIPH_PARM_PERFORM_CTR:
    if (value == 0 || value == 1) {
      if (value == 0 &&
        ...
        ACVP_LOG_WARN("Perform counter test set to false, but value for ctr "
                      "increment or ctr overflow already set. Server will "
                      "ignore other values. Continuing...");
      }
      cap->cap.sym_cap->perform_ctr_tests = value;
      return ACVP_SUCCESS;
    } else {
      ACVP_LOG_ERR(
          "Invalid parameter 'value' for param ACVP_SYM_CIPH_PARM_PERFORM_CTR");
      return ACVP_INVALID_ARG;
    }
  ...
  case ACVP_SYM_CIPH_PARM_CTR_INCR:
    if (cap->cap.sym_cap->perform_ctr_tests == 0) {
      ACVP_LOG_WARN(
          "Perform counter test set to false, but value for ctr increment "
    ...
    if (value == 0 || value == 1) {
      ...
      ACVP_LOG_ERR(
          "Invalid parameter 'value' for param ACVP_SYM_CIPH_PARM_CTR_INCR");
      return ACVP_INVALID_ARG;
    }
  ...
  case ACVP_SYM_CIPH_PARM_CTR_OVRFLW:
    if (cap->cap.sym_cap->perform_ctr_tests == 0) {
      ACVP_LOG_WARN(
          "Perform counter test set to false, but value for ctr overflow being "
          "set; server will ignore this. Continuing...");
    }
    if (value == 0 || value == 1) {
      ...
      ACVP_LOG_ERR(
          "Invalid parameter 'value' for param ACVP_SYM_CIPH_PARM_CTR_OVRFLW");
      return ACVP_INVALID_ARG;
    }
  ...
  case ACVP_SYM_CIPH_PARM_IVGEN_SRC:
    if (value > 0 && value < ACVP_SYM_CIPH_IVGEN_SRC_MAX) {
      if (value == ACVP_SYM_CIPH_IVGEN_SRC_EITHER) {
        /* This will generate two vector sets, one for internal ivgen and one
         * for external */
        ctx->vs_count++;
      }
      ...
      ACVP_LOG_ERR(
          "Invalid parameter 'value' for param ACVP_SYM_CIPH_PARM_IVGEN_SRC");
      return ACVP_INVALID_ARG;
    }
  ...
  case ACVP_SYM_CIPH_PARM_IVGEN_MODE:
    if (value > 0 && value < ACVP_SYM_CIPH_IVGEN_MODE_MAX) {
      ...
      ACVP_LOG_ERR(
          "Invalid parameter 'value' for param ACVP_SYM_CIPH_PARM_IVGEN_MODE");
      return ACVP_INVALID_ARG;
    }
  case ACVP_SYM_CIPH_PARM_SALT_SRC:
    if (cipher == ACVP_AES_XPN && value > 0 &&
      ...
      ACVP_LOG_ERR(
          "Invalid parameter 'value' for parm ACVP_SYM_CIPH_PARM_SALT_SRC");
      return ACVP_INVALID_ARG;
    }
  case ACVP_SYM_CIPH_PARM_CONFORMANCE:
    if (cipher == ACVP_AES_CTR && value == ACVP_CONFORMANCE_RFC3686) {
      ...
      ACVP_LOG_ERR(
          "Invalid parameter 'value' for parm ACVP_SYM_CIPH_PARM_CONFORMANCE");
      return ACVP_INVALID_ARG;
    }
  case ACVP_SYM_CIPH_PARM_DULEN_MATCHES_PAYLOADLEN:
    if (cipher != ACVP_AES_XTS) {
      ACVP_LOG_ERR("ACVP_SYM_CIPH_PARM_DULEN_MATCHES_PAYLOADLEN can only be "
                   "set for AES-XTS");
      return ACVP_INVALID_ARG;
    }
    if ((cap->cap.sym_cap->du_len.max != 0 ||
         cap->cap.sym_cap->du_len.increment != 0)) {
      ACVP_LOG_ERR("ACVP_SYM_CIPH_DULEN_MATCHES_PAYLOADLEN cannot be changed "
                   "after setting "
  ...
  case ACVP_SYM_CIPH_IVLEN:
    if (acvp_is_domain_already_set(&cap->cap.sym_cap->iv_len)) {
      ACVP_LOG_ERR(
          "ivLen already defined using acvp_sym_cipher_set_domain. Please set "
          "ivLen using only one function "
          "(Using set_parm for ivLen will eventually be depreciated).");
      return ACVP_INVALID_ARG;
    }
  ...
  case ACVP_SYM_CIPH_PTLEN:
    if (acvp_is_domain_already_set(&cap->cap.sym_cap->payload_len)) {
      ACVP_LOG_ERR(
          "payloadLen already defined using acvp_sym_cipher_set_domain. Please "
          "set payloadLen using only one function "
  ...
  case ACVP_SYM_CIPH_AADLEN:
    if (acvp_is_domain_already_set(&cap->cap.sym_cap->aad_len)) {
      ACVP_LOG_ERR(
          "aadLen already defined using acvp_sym_cipher_set_domain. Please set "
          "aadLen using only one function "
          "(Using set_parm for aadLen will eventually be depreciated).");
      return ACVP_INVALID_ARG;
    }
...
ACVP_RESULT acvp_cap_hash_set_domain(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  case ACVP_HASH_OUT_LENGTH:
    ...
    if (min < ACVP_HASH_XOF_MD_BIT_MIN || max > ACVP_HASH_XOF_MD_BIT_MAX) {
      ACVP_LOG_ERR(
          "'ACVP_HASH_OUT_LENGTH' min or max outside of acceptable range");
      return ACVP_INVALID_ARG;
    }
...
static ACVP_RESULT acvp_validate_hmac_parm_value(ACVP_CIPHER cipher,
  ...
  case ACVP_HMAC_MACLEN:
    ...
    if (value >= ACVP_HMAC_MAC_BIT_MIN && value <= max_val && value % 8 == 0) {
      retval = ACVP_SUCCESS;
    }
    break;
...
static ACVP_RESULT acvp_validate_drbg_parm_value(ACVP_DRBG_PARM parm,
  ...
  case ACVP_DRBG_ENTROPY_LEN:
    ...
    break;
  case ACVP_DRBG_NONCE_LEN:
    if (value >= ACVP_DRBG_NONCE_BIT_MIN && value <= ACVP_DRBG_NONCE_BIT_MAX) {
      retval = ACVP_SUCCESS;
    }
    break;
  case ACVP_DRBG_PERSO_LEN:
...
ACVP_RESULT acvp_cap_drbg_set_parm(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  if (acvp_validate_drbg_parm_value(param, value) != ACVP_SUCCESS) {
    ACVP_LOG_ERR(
        "Error validating DRBG paramater value (param = %d, value = %d", param,
        value);
    return ACVP_INVALID_ARG;
  }
...
ACVP_RESULT acvp_cap_rsa_keygen_set_parm(ACVP_CTX *ctx, ACVP_RSA_PARM param,
  ...
  case ACVP_RSA_PARM_FIXED_PUB_EXP_VAL:
    ...
    ACVP_LOG_ERR("Use acvp_enable_rsa_keygen_mode() or "
                 "acvp_enable_rsa_keygen_exp_parm() API to enable a new randPQ "
                 "or exponent.");
    break;
  default:
    rv = ACVP_INVALID_ARG;
...
acvp_cap_rsa_sig_enable(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  if (result == ACVP_DUP_CIPHER) {
    ...
    ACVP_LOG_ERR("Failed to allocate (%s) capability object", cap_message_str);
  }
...
ACVP_RESULT acvp_cap_rsa_prim_set_parm(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  case ACVP_RSA_PARM_PUB_EXP_MODE:
    if (cipher == ACVP_RSA_DECPRIM &&
      ...
      ACVP_LOG_ERR("RSA decryption primitive public exponent mode can only be "
                   "set for default revisions");
      rv = ACVP_INVALID_ARG;
      break;
    }
  ...
  case ACVP_RSA_PARM_KEY_FORMAT:
    if (cipher == ACVP_RSA_DECPRIM &&
        cap_list->cap.rsa_prim_cap->revision != 0) {
      ACVP_LOG_ERR("RSA decryption primitive key format can only be set for "
                   "default revisions");
      rv = ACVP_INVALID_ARG;
      break;
    }
  ...
  case ACVP_RSA_PARM_MODULO:
    ...
    if (value != 2048 && value != 3072 && value != 4096) {
      ACVP_LOG_ERR("Only modulus values of 2048, 3072, or 4096 are supported "
                   "for RSA primitives");
      rv = ACVP_INVALID_ARG;
      break;
    }
...
ACVP_RESULT acvp_cap_rsa_prim_set_exponent(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  if (cipher == ACVP_RSA_DECPRIM && cap_list->cap.rsa_prim_cap->revision != 0) {
    ACVP_LOG_ERR("RSA decryption primitive public exponent can only be set for "
                 "default revision");
    return ACVP_INVALID_ARG;
  }
...
ACVP_RESULT acvp_cap_ecdsa_set_parm(ACVP_CTX *ctx, ACVP_CIPHER cipher,
    ...
    if (cipher == ACVP_ECDSA_SIGGEN || cipher == ACVP_ECDSA_SIGVER ||
      ...
      if (value >= ACVP_ECDSA_COMPONENT_MODE_NO &&
        ...
        if (value == ACVP_ECDSA_COMPONENT_MODE_BOTH) {
          /* This will generate two vector sets, one for and one not for
           * component mode */
          ctx->vs_count++;
        }
        cap->component = value;
  ...
  case ACVP_ECDSA_REVISION:
    ...
    if (value != ACVP_REVISION_FIPS186_4) {
      ACVP_LOG_ERR("Invalid ECDSA revision. Only revision FIPS 186-4 can be "
                   "set for ECDSA. default is 186-5.");
      return ACVP_INVALID_ARG;
    }
    cap->revision = value;
    break;
...
acvp_cap_kdf135_ikev2_enable(ACVP_CTX *ctx,
  ...
  result = acvp_cap_list_append(ctx, ACVP_KDF135_IKEV2_TYPE, ACVP_KDF135_IKEV2,
                                crypto_handler);
...
acvp_cap_kdf135_ikev1_enable(ACVP_CTX *ctx,
  ...
  result = acvp_cap_list_append(ctx, ACVP_KDF135_IKEV1_TYPE, ACVP_KDF135_IKEV1,
                                crypto_handler);
...
acvp_cap_kdf108_enable(ACVP_CTX *ctx,
  ...
  result =
      acvp_cap_list_append(ctx, ACVP_KDF108_TYPE, ACVP_KDF108, crypto_handler);
...
ACVP_RESULT acvp_cap_kdf108_set_parm(ACVP_CTX *ctx, ACVP_KDF108_MODE mode,
  ...
  case ACVP_KDF108_MAC_MODE:
    ...
    case ACVP_KDF108_MAC_MODE_CMAC_AES128:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_CMAC_AES_128);
      break;
    case ACVP_KDF108_MAC_MODE_CMAC_AES192:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_CMAC_AES_192);
      break;
    case ACVP_KDF108_MAC_MODE_CMAC_AES256:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_CMAC_AES_256);
      break;
    ...
    case ACVP_KDF108_MAC_MODE_HMAC_SHA224:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA2_224);
      break;
    case ACVP_KDF108_MAC_MODE_HMAC_SHA256:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA2_256);
      break;
    case ACVP_KDF108_MAC_MODE_HMAC_SHA384:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA2_384);
      break;
    case ACVP_KDF108_MAC_MODE_HMAC_SHA512:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA2_512);
      break;
    ...
    case ACVP_KDF108_MAC_MODE_HMAC_SHA3_224:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA3_224);
      break;
    case ACVP_KDF108_MAC_MODE_HMAC_SHA3_256:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA3_256);
      break;
    case ACVP_KDF108_MAC_MODE_HMAC_SHA3_384:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA3_384);
      break;
    case ACVP_KDF108_MAC_MODE_HMAC_SHA3_512:
      result =
          acvp_append_name_list(&mode_obj->mac_mode, ACVP_ALG_HMAC_SHA3_512);
      break;
...
ACVP_RESULT acvp_cap_kdf135_x942_set_domain(ACVP_CTX *ctx,
  ...
  if (acvp_validate_kdf135_x942_domain_value(param, min, max, increment) !=
      ACVP_SUCCESS) {
    ACVP_LOG_ERR("Invalid domain range provided for param %d in kdf135-x942",
                 param);
    return ACVP_INVALID_ARG;
  }
...
ACVP_RESULT acvp_cap_kdf108_set_domain(ACVP_CTX *ctx, ACVP_KDF108_MODE mode,
  ...
  if (param != ACVP_KDF108_SUPPORTED_LEN && increment % 8 != 0) {
    ACVP_LOG_ERR(
        "Domain increments must be divisible into bytes for KDF108 (given: %d)",
        increment);
    return ACVP_INVALID_ARG;
  }
...
ACVP_RESULT acvp_cap_kdf_tls12_set_parm(ACVP_CTX *ctx,
  ...
  case ACVP_KDF_TLS12_HASH_ALG:
    ...
    if (acvp_is_in_name_list(cap->hash_algs, alg_str)) {
      ACVP_LOG_WARN("Attempting to register a hash alg with TLS 1.2 KDF that "
...
ACVP_RESULT acvp_cap_kdf_tls13_set_parm(ACVP_CTX *ctx,
  ...
  case ACVP_KDF_TLS13_HMAC_ALG:
    ...
    if (acvp_is_in_name_list(cap->hmac_algs, alg_str)) {
      ACVP_LOG_WARN("Attempting to register an hmac alg with TLS 1.3 KDF that "
                    "has already been registered, skipping.");
      return ACVP_SUCCESS;
    } else {
      result = acvp_append_name_list(&cap->hmac_algs, alg_str);
 ...
 */
static ACVP_RESULT
acvp_add_kas_ecc_prereq_val(ACVP_CTX *ctx, ACVP_KAS_ECC_CAP_MODE *kas_ecc_mode,
 ...
 */
static ACVP_RESULT
acvp_add_kas_ffc_prereq_val(ACVP_CTX *ctx, ACVP_KAS_FFC_CAP_MODE *kas_ffc_mode,
...
ACVP_RESULT acvp_cap_kda_set_parm(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  if (param == ACVP_KDA_PATTERN && value == ACVP_KDA_PATTERN_LITERAL &&
      !string) {
    ACVP_LOG_ERR("string must not be null when setting literal pattern for KDA "
                 "algorithms.");
    return ACVP_INVALID_ARG;
  }
  if (string &&
      (param != ACVP_KDA_PATTERN || value != ACVP_KDA_PATTERN_LITERAL)) {
    ACVP_LOG_WARN("String parameter should only be used when setting literal "
                  "pattern. Ignoring value...");
  }
  ...
  case ACVP_SUB_KDA_ONESTEP:
    ...
    case ACVP_KDA_PATTERN:
      if (value == ACVP_KDA_PATTERN_LITERAL &&
        ...
        ACVP_LOG_WARN("Literal pattern candidate was already previously set. "
                      "Replacing...");
      ...
      if (value > ACVP_KDA_PATTERN_NONE && value < ACVP_KDA_PATTERN_MAX) {
        ...
        ACVP_LOG_ERR("Invalid pattern type specified when setting param for "
                     "KDA onestep.");
        return ACVP_INVALID_ARG;
      }
      break;
    ...
    case ACVP_KDA_ENCODING_TYPE:
      if (value > ACVP_KDA_ENCODING_NONE && value < ACVP_KDA_ENCODING_MAX) {
        ...
        ACVP_LOG_ERR("Invalid encoding type specified when setting param for "
                     "KDA onestep.");
        return ACVP_INVALID_ARG;
      }
      break;
    case ACVP_KDA_L:
      if (value <= 0) {
        ...
        ACVP_LOG_ERR("Value for l for KDA onestep must be convertable to exact "
                     "bytes (mod 8)");
        return ACVP_INVALID_ARG;
      } else {
        os_cap->l = value;
  ...
  case ACVP_SUB_KDA_HKDF:
    ...
    case ACVP_KDA_PATTERN:
      ...
      if (value > ACVP_KDA_PATTERN_NONE && value < ACVP_KDA_PATTERN_MAX) {
        ...
        ACVP_LOG_ERR(
            "Invalid pattern type specified when setting param for KDA-HKDF.");
        return ACVP_INVALID_ARG;
      }
      break;
    case ACVP_KDA_REVISION:
    ...
    case ACVP_KDA_ENCODING_TYPE:
      if (value > ACVP_KDA_ENCODING_NONE && value < ACVP_KDA_ENCODING_MAX) {
        ...
        ACVP_LOG_ERR(
            "Invalid encoding type specified when setting param for KDA-HKDF.");
        return ACVP_INVALID_ARG;
      }
      break;
    case ACVP_KDA_L:
      if (value <= 0) {
        ...
        ACVP_LOG_ERR("Value for l for KDA-HKDF must be convertable to exact "
                     "bytes (mod 8)");
        return ACVP_INVALID_ARG;
      } else {
        hkdf_cap->l = value;
    ...
    case ACVP_KDA_USE_HYBRID_SECRET:
      ...
      if (cap_list->cap.kda_hkdf_cap->revision) {
        ACVP_LOG_ERR(
            "Hybrid secrets for HKDF can only be set for revision SP800-56Cr2");
        return ACVP_INVALID_ARG;
      }
...
ACVP_RESULT acvp_cap_kda_twostep_set_parm(ACVP_CTX *ctx, ACVP_KDA_PARM param,
  ...
  case ACVP_KDA_PATTERN:
    if (value == ACVP_KDA_PATTERN_LITERAL && cap->literal_pattern_candidate) {
      ACVP_LOG_WARN(
          "Literal pattern candidate was already previously set. Replacing...");
    ...
    if (value > ACVP_KDA_PATTERN_NONE && value < ACVP_KDA_PATTERN_MAX) {
      ...
      ACVP_LOG_ERR(
          "Invalid pattern type specified when setting param for KDA twostep.");
      return ACVP_INVALID_ARG;
    }
    break;
  case ACVP_KDA_REVISION:
  ...
  case ACVP_KDA_ENCODING_TYPE:
    if (value > ACVP_KDA_ENCODING_NONE && value < ACVP_KDA_ENCODING_MAX) {
      ...
      ACVP_LOG_ERR("Invalid encoding type specified when setting param for KDA "
                   "twostep.");
      return ACVP_INVALID_ARG;
    }
    break;
  case ACVP_KDA_L:
    if (value <= 0) {
      ...
      ACVP_LOG_ERR("Value for l for KDA twostep must be convertable to exact "
                   "bytes (mod 8)");
      return ACVP_INVALID_ARG;
    } else {
      cap->l = value;
  ...
  case ACVP_KDA_USE_HYBRID_SECRET:
    if (cap_list->cap.kda_twostep_cap->revision) {
      ACVP_LOG_ERR("Hybrid secrets for twostep can only be set for revision "
                   "SP800-56Cr2");
      return ACVP_INVALID_ARG;
    }
    acvp_append_sl_list(&cap_list->cap.kda_twostep_cap->aux_secret_len.values,
                        value);
  ...
  case ACVP_KDA_TWOSTEP_REQUIRES_EMPTY_IV:
    if (mode_obj->empty_iv_support == 0) {
      ACVP_LOG_ERR("REQUIRES_EMPTY_IV for twostep modes can only be set if "
                   "SUPPORTS_EMPTY_IV is true");
      return ACVP_INVALID_ARG;
    } else {
      mode_obj->requires_empty_iv = value;
...
acvp_cap_lms_enable(ACVP_CTX *ctx, ACVP_CIPHER cipher,
  ...
  case ACVP_SUB_LMS_KEYGEN:
    result =
        acvp_cap_list_append(ctx, ACVP_LMS_KEYGEN_TYPE, cipher, crypto_handler);
    break;
  case ACVP_SUB_LMS_SIGGEN:
    result =
        acvp_cap_list_append(ctx, ACVP_LMS_SIGGEN_TYPE, cipher, crypto_handler);
    break;
  case ACVP_SUB_LMS_SIGVER:
    result =
        acvp_cap_list_append(ctx, ACVP_LMS_SIGVER_TYPE, cipher, crypto_handler);
    break;
  default:
