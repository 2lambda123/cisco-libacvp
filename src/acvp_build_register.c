...
acvp_build_rsa_keygen_register_cap(JSON_Object *cap_obj,
  ...
  while (keygen_cap) {
    ...
    json_object_set_string(alg_specs_obj, "randPQ",
...
static ACVP_RESULT acvp_build_rsa_sig_register_cap(JSON_Object *cap_obj,
  ...
  while (rsa_cap_mode) {
    alg_specs_val = json_value_init_object();
    alg_specs_obj = json_value_get_object(alg_specs_val);
    json_object_set_string(alg_specs_obj, "sigType",
                           rsa_cap_mode->sig_type_str);
    ...
    while (current_sig_type_cap) {
      ...
      while (current_hash_pair) {
        ...
        json_object_set_string(hash_pair_obj, "hashAlg",
                               current_hash_pair->name);
        strncmp_s(rsa_cap_mode->sig_type_str, ACVP_RSA_SIG_TYPE_LEN_MAX, "pss",
                  3, &diff);
        if (!diff) {
...
static ACVP_RESULT acvp_build_rsa_prim_register_cap(JSON_Object *cap_obj,
  ...
  if (cap_entry->cap.rsa_prim_cap->revision) {
    revision =
        acvp_lookup_alt_revision_string(cap_entry->cap.rsa_prim_cap->revision);
  } else {
    revision = acvp_lookup_cipher_revision(cap_entry->cipher);
  }
  if (revision == NULL)
  ...
  if (cap_entry->cipher == ACVP_RSA_SIGPRIM) {
    ...
    json_array_append_string(arr, prim_cap->pub_exp_mode ==
                                          ACVP_RSA_PUB_EXP_MODE_FIXED
                                      ? ACVP_RSA_PUB_EXP_MODE_FIXED_STR
                                      : ACVP_RSA_PUB_EXP_MODE_RANDOM_STR);
    if (prim_cap->pub_exp_mode == ACVP_RSA_PUB_EXP_MODE_FIXED) {
...
static ACVP_RESULT acvp_build_ecdsa_register_cap(ACVP_CTX *ctx,
  ...
  case ACVP_SUB_ECDSA_SIGGEN:
    ...
    current_curve = ecdsa_cap->curves;
    // add "universally" set hash algs here instead of later to be resliant to
    // different combos of API calls
    while (current_curve) {
  ...
  /**
   * hashAlgs is relatively complicated. We want to compare every curve that is
   * registered and the hash algs registered to them. If they have the same hash
   * algs, we put them in the same object within the capabilities array. If they
   * have different hash algs, a new object is created.
   */
  if (cipher == ACVP_ECDSA_SIGGEN || cipher == ACVP_ECDSA_SIGVER ||
    ...
    while (current_curve) {
      ...
      // One of these vals for every object in the array - every object being
      // list of curves that share the same hashAlgs
      ...
      // Now, check every curve on the list aftetwards, and memcmp to see if it
      // has the same hashAlgs, appending it to the same obj when applicable
      iter = current_curve->next;
      while (iter) {
        // If the curve is already accounted for by a previous current_curve,
        // skip
        if (current_curve->curve == iter->curve || track[iter->curve]) {
          iter = iter->next;
          continue;
        }
        memcmp_s(current_curve->algs, sizeof(current_curve->algs), iter->algs,
                 sizeof(iter->algs), &diff);
        if (!diff) {
          // if they have the same algs arrays, they go in the same obj in the
          // capabilities array
...
static ACVP_RESULT acvp_build_kdf108_register_cap(JSON_Object *cap_obj,
  ...
  if (cap_entry->cap.kdf108_cap->counter_mode.kdf_mode) {
    ...
    acvp_build_kdf108_mode_register(&alg_specs_counter_obj,
                                    &cap_entry->cap.kdf108_cap->counter_mode);
  ...
  if (cap_entry->cap.kdf108_cap->feedback_mode.kdf_mode) {
    ...
    acvp_build_kdf108_mode_register(&alg_specs_feedback_obj,
                                    &cap_entry->cap.kdf108_cap->feedback_mode);
...
static ACVP_RESULT acvp_build_pbkdf_register_cap(JSON_Object *cap_obj,
  ...
  tmp_obj = json_value_get_object(tmp_val);
  json_object_set_number(tmp_obj, "min",
                         cap_entry->cap.pbkdf_cap->iteration_count_domain.min);
  json_object_set_number(tmp_obj, "max",
                         cap_entry->cap.pbkdf_cap->iteration_count_domain.max);
  json_object_set_number(
      tmp_obj, "increment",
  ...
  // create the "passwordLen" array within the "capabilities" array and populate
  // it
  ...
  if (kas_ecc_mode->prereq_vals) {
    ...
    case ACVP_KAS_ECC_MODE_NONE:
      ...
      json_object_set_value(cap_obj, "domainParameterGenerationMethods",
                            json_value_init_array());
      temp_arr =
          json_object_get_array(cap_obj, "domainParameterGenerationMethods");
      current_curve = kas_ecc_mode->curve;
      while (current_curve) {
        const char *curve_str = NULL;
      ...
      while (current_scheme) {
        ...
        while (current_pset) {
          ...
          curve_str = acvp_lookup_ec_curve_name(kas_ecc_cap->cipher,
                                                current_pset->curve);
          if (!curve_str) {
            ACVP_LOG_ERR("Unsupported curve %d", current_pset->curve);
            return ACVP_INVALID_ARG;
          }
...
acvp_build_kda_onestep_register_cap(ACVP_CTX *ctx, JSON_Object *cap_obj,
  ...
  if (!revision) {
    ACVP_LOG_ERR("Unable to find revision string for KDA-ONESTEP when building "
                 "registration");
    rv = ACVP_INVALID_ARG;
    goto err;
  }
  ...
  while (tmp_param_list) {
    ...
    case ACVP_KDA_PATTERN_T:
      strncat_s(pattern_str, ACVP_KDA_PATTERN_REG_STR_MAX + 1,
                ACVP_KDA_PATTERN_T_STR, sizeof(ACVP_KDA_PATTERN_T_STR) - 1);
      break;
    default:
...
acvp_build_kda_twostep_register_cap(ACVP_CTX *ctx, JSON_Object *cap_obj,
  ...
  if (!revision) {
    ACVP_LOG_ERR("Unable to find revision string for KDA-TWOSTEP when building "
                 "registration");
    rv = ACVP_INVALID_ARG;
    goto err;
  }
  ...
  /* Append the "usesHybridShareSecret" value and "auxSharedSecretLen" value if
   * enabled */
  if (cap_entry->cap.kda_twostep_cap->use_hybrid_shared_secret) {
    ...
    if (cap_entry->cap.kda_twostep_cap->aux_secret_len.min != 0 ||
      ...
      tmp_obj = json_value_get_object(tmp_val);
      json_object_set_number(
          tmp_obj, "min", cap_entry->cap.kda_twostep_cap->aux_secret_len.min);
      json_object_set_number(
          tmp_obj, "max", cap_entry->cap.kda_twostep_cap->aux_secret_len.max);
      json_object_set_number(
          tmp_obj, "increment",
...
static ACVP_RESULT acvp_build_kda_hkdf_register_cap(ACVP_CTX *ctx,
  ...
  if (!mode) {
    ACVP_LOG_ERR(
        "Unable to find mode string for KDA-HKDF when building registration");
    rv = ACVP_INVALID_ARG;
    goto err;
  }
  json_object_set_string(cap_obj, "mode", mode);
  ...
  if (!revision) {
    ACVP_LOG_ERR("Unable to find revision string for KDA-HKDF when building "
                 "registration");
    rv = ACVP_INVALID_ARG;
    goto err;
  }
...
ACVP_RESULT acvp_build_registration_json(ACVP_CTX *ctx, JSON_Value **reg) {
  ...
  if (ctx->caps_list) {
    ...
    while (cap_entry) {
      ...
      case ACVP_AES_CTR:
        /**
         * If we need to test both internal and external IV gen, we need two
         * different algorithm registrations/vector sets currently.
         */
        if (cap_entry->cap.sym_cap->ivgen_source ==
          ...
          if (rv != ACVP_SUCCESS) {
            cap_entry->cap.sym_cap->ivgen_source =
                ACVP_SYM_CIPH_IVGEN_SRC_EITHER;
            break;
          }
      ...
      case ACVP_ECDSA_KEYGEN:
      case ACVP_ECDSA_KEYVER:
        rv = acvp_build_ecdsa_register_cap(ctx, cap_entry->cipher, cap_obj,
                                           cap_entry);
        break;
      case ACVP_ECDSA_SIGGEN:
      ...
      case ACVP_ECDSA_SIGVER:
        /* If component_test = BOTH, we need two registrations */
        if (cap_entry->cap.ecdsa_sigver_cap->component ==
            ACVP_ECDSA_COMPONENT_MODE_BOTH) {
          cap_entry->cap.ecdsa_sigver_cap->component =
              ACVP_ECDSA_COMPONENT_MODE_NO;
          rv = acvp_build_ecdsa_register_cap(ctx, cap_entry->cipher, cap_obj,
                                             cap_entry);
          if (rv != ACVP_SUCCESS) {
