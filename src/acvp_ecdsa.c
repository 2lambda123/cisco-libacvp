...
static ACVP_RESULT acvp_ecdsa_kat_handler_internal(ACVP_CTX *ctx,
                                                   JSON_Object *obj,
                                                   ACVP_CIPHER cipher);
 ...
 */
static ACVP_RESULT acvp_ecdsa_output_tc(ACVP_CTX *ctx, ACVP_CIPHER cipher,
                                        ACVP_ECDSA_TC *stc,
                                        JSON_Object *tc_rsp) {
  ACVP_RESULT rv;
  char *tmp = NULL;
...
static ACVP_RESULT
acvp_ecdsa_init_tc(ACVP_CTX *ctx, ACVP_CIPHER cipher, int is_component,
                   ACVP_ECDSA_TC *stc, int tg_id, unsigned int tc_id,
                   ACVP_EC_CURVE curve,
                   ACVP_ECDSA_SECRET_GEN_MODE secret_gen_mode,
                   ACVP_HASH_ALG hash_alg, const char *qx, const char *qy,
                   const char *message, const char *r, const char *s) {
  ...
  if (cipher == ACVP_ECDSA_SIGVER || cipher == ACVP_ECDSA_SIGGEN ||
    ...
    rv = acvp_hexstr_to_bin(message, stc->message, ACVP_RSA_MSGLEN_MAX,
                            &(stc->msg_len));
    if (rv != ACVP_SUCCESS) {
...
static ACVP_ECDSA_SECRET_GEN_MODE read_secret_gen_mode(const char *str) {
  ...
  strcmp_s(ACVP_ECDSA_EXTRA_BITS_STR, ACVP_ECDSA_EXTRA_BITS_STR_LEN, str,
           &diff);
  if (!diff)
    return ACVP_ECDSA_SECRET_GEN_EXTRA_BITS;

  strcmp_s(ACVP_ECDSA_TESTING_CANDIDATES_STR,
           ACVP_ECDSA_TESTING_CANDIDATES_STR_LEN, str, &diff);
  if (!diff)
    return ACVP_ECDSA_SECRET_GEN_TEST_CAND;
...
static ACVP_RESULT acvp_ecdsa_kat_handler_internal(ACVP_CTX *ctx,
  ...
  const char *alg_str, *mode_str, *qx = NULL, *qy = NULL, *r = NULL, *s = NULL,
                                  *message = NULL;
  ...
  for (i = 0; i < g_cnt; i++) {
    ...
    if (alg_id == ACVP_ECDSA_KEYGEN) {
      secret_gen_mode_str =
          json_object_get_string(groupobj, "secretGenerationMode");
      ...
      if (!hash_alg ||
          ((alg_id == ACVP_ECDSA_SIGGEN || alg_id == ACVP_DET_ECDSA_SIGGEN) &&
           hash_alg == ACVP_SHA1)) {
    ...
    for (j = 0; j < t_cnt; j++) {
      ...
      if (alg_id == ACVP_ECDSA_KEYVER || alg_id == ACVP_ECDSA_SIGVER) {
        ...
        if (strnlen_s(qx, ACVP_ECDSA_EXP_LEN_MAX + 1) >
                ACVP_ECDSA_EXP_LEN_MAX ||
            strnlen_s(qy, ACVP_ECDSA_EXP_LEN_MAX + 1) >
                ACVP_ECDSA_EXP_LEN_MAX) {
      ...
      if (alg_id == ACVP_ECDSA_SIGGEN || alg_id == ACVP_ECDSA_SIGVER ||
        ...
        if (strnlen_s(message, ACVP_ECDSA_MSGLEN_MAX + 1) >
            ACVP_ECDSA_MSGLEN_MAX) {
      ...
      rv = acvp_ecdsa_init_tc(ctx, alg_id, is_component, &stc, tgId, tc_id,
                              curve, secret_gen_mode, hash_alg, qx, qy, message,
                              r, s);
      ...
      if (cipher == ACVP_ECDSA_SIGGEN || cipher == ACVP_DET_ECDSA_SIGGEN) {
        ...
        rv =
            acvp_bin_to_hexstr(stc.qy, stc.qy_len, tmp, ACVP_ECDSA_EXP_LEN_MAX);
        if (rv != ACVP_SUCCESS) {
          ACVP_LOG_ERR("hex conversion failure (qy)");
          free(tmp);
          json_value_free(r_tval);
        ...
        rv =
            acvp_bin_to_hexstr(stc.qx, stc.qx_len, tmp, ACVP_ECDSA_EXP_LEN_MAX);
        if (rv != ACVP_SUCCESS) {
          ACVP_LOG_ERR("hex conversion failure (qx)");
          free(tmp);
          json_value_free(r_tval);
